...
def submit(url,data,ID):
    ...
    headers = {'cookie':str(submitCookie),'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36'}
    if ID==1:
...
class CdmaoStudio():
    ...
    def get_Discussions(self):
        '''获取工作室评论'''
        rr = get('https://api.codemao.cn/web/discussions/'+str(self.id)+'/comments?source=WORK_SHOP&sort=-created_at&limit=20&offset=0')
        idd=[]
        nickname=[]
        avatar_url=[]
        position=[]
