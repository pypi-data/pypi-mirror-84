...
class FinalEnd(RootBaseState):
    ...
    def startswith(self, line):
        match = re.compile(r'Computation time\s+([0-9]*[.])?[0-9]+([eE][-+]?\d+)?\s+(seconds|minutes)\s+Maximum cascade storage(\s*\d+|\*{6})')
        return match.fullmatch(line)
...
class End(BaseState):
    ...
    def startswith(self, line):
        match = re.compile(r'End of program:  total time\s+([0-9]*[.])?[0-9]+([eE][-+]?\d+)?\s+(seconds|minutes)')
        return match.fullmatch(line)
