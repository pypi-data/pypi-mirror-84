...
class HBaseEs(object):
    ...
    def __es_bulk(self, _op_type, index, data_list):
        """
        es批量操作方法
        :param _op_type: 操作类型
        :param index: 相应索引
    ...
    def add(self, index, prefix="", p_key_name="id", data_list=[], is_cover=1):
        """
         :param index:索引
         :param prefix:前缀:dw,dm,ods
        :param p_key_name:
        ...
        self.hbase.batch_put(row_key_name=p_key_name, index=index, column=COLUMN, datas=data_hbase_list, prefix=prefix,
                             batch_size=2000)
    ...
    def update(self, index, prefix="", p_key_name="id", data_list=[], is_cover=0, only_es=0,
        ...
        if is_cover == 1:
            ...
            if exist_update_not_exist_index == 1:
                ...
                bulk_list.extend(self.__es_bulk('index', index, data_list_not_exit_list))
                bulk_list.extend(self.__es_bulk('update', index, data_list_exit_list))
        ...
        if only_es == 1:
            ...
            self.hbase.batch_put(row_key_name=p_key_name, index=index, column=COLUMN, datas=data_hbase_list,
                                 prefix=prefix,
                                 batch_size=2000)
        bulk_list = self.__es_bulk(_op_type, index, data_list)
        helpers.bulk(self.es, bulk_list)

        return {
            'msg': 'update successful', 'status': 200}

    def delete(self, index, prefix="", p_key_name="id", data_list=[]):
        """
        :param index:索引
         :param prefix:前缀:dw,dm,ods
        :param p_key_name:
