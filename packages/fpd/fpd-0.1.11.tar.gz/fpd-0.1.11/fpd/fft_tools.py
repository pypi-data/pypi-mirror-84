...
def fft2_diff(im, ypix=None, xpix=None, order=1):
    ...
    ky, kx = [fftfreq(si, pi) for (si, pi) in zip(im.shape, (ypix, xpix))]
    if order < 0:
        ky[0] = 1
        kx[0] = 1
    ...
    if order < 0:
        xmul[0, 0] = 0
        ymul[0, 0] = 0
    im_gradx = ifft2(xmul).real
    im_grady = ifft2(ymul).real
...
def fft2_igrad(grady, gradx, ypix=None, xpix=None):
    ...
    ky, kx = [fftfreq(si, pi) for (si, pi) in zip(grady.shape, (ypix, xpix))]
    ...
    im_ft = (fft2(grady) * (kyy * 1j) + fft2(gradx) * (kxx * 1j)) / delsq
    im_ft[0, 0] = 0
    im = ifft2(im_ft).real
    ...
    #xmul = fft2(gradx) * (kxx * 1j)**-1
    #ymul = fft2(grady) * (kyy * 1j)**-1
    #xmul[0, 0] = 0
    #ymul[0, 0] = 0
    #im = ifft2(ymul).real + ifft2(xmul).real
    return im
...
def cepstrum2(image, mode='real', sigma=1, pad_to=None, plot=False):
    ...
    imagef = np.fft.fft2(image)
    imagefc = np.log(np.abs(imagef)**2)
    cp = np.abs(np.fft.ifft2(imagefc))**2
    cp = np.fft.fftshift(cp)
