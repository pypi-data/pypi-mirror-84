...
from tethys_utils.data_models import Geometry, Dataset, DatasetBase, S3ObjectKey, Station
...
def process_real_station(dataset_id, coords, df, parameter, precision, s3, bucket, key, geo_type='Point', ref=None, name=None, osm_id=None, altitude=None, properties=None):
    ...
    station_m = Station(dataset_id=dataset_id, station_id=station_id, virtual_station=False, geometry=geo1, stats=stats1, time_series_object_info=object_info1, ref=ref, name=name, osm_id=osm_id, altitude=altitude, properties=properties)
...
def get_qc_hilltop_data(param, ts_local_tz, station_mtype_corrections):
    ...
    try:
        ...
        encoding = {'quality_code': {'dtype': 'uint16', '_FillValue': -9999}}
        ...
        stns2.rename(columns={'SiteName': 'station_id'}, inplace=True)
            ...
            if not mtypes_df2.empty:
                ...
                for i, row in mtypes_df2.iterrows():
                    ...
                    for ds in datasets[meas]:
                        ...
                        ## Get stn_id
                        stn2 = stns2[stns2.station_id == row.Site].copy()
                        ...
                        if not up1.empty:
                            ...
                            df4_up = pd.merge(up1, stn2.drop(['geo', 'station_id'], axis=1), on='station_id')
                            df4_all = pd.merge(df3, stn2.drop(['geo', 'station_id'], axis=1), on='station_id')
                            ...
                            ## Process stn data
                            stn3 = stn2.iloc[0]
                            stn_m = process_real_station(ds['dataset_id'], [float(stn3.lon), float(stn3.lat)], df3, parameter, precision, s3, param['remote']['bucket'], last_key, ref=stn3['station_id'])
