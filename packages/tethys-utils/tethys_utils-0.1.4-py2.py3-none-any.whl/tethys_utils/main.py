...
from tethys_utils.data_models import Geometry, Dataset, DatasetBase
...
nc_ts_key_pattern = {
                    'H23': 'time_series/{owner}/{feature}/{parameter}/{method}/{processing_code}/{aggregation_statistic}/{frequency_interval}/{utc_offset}/{date}/{site}.H23.nc',
                    'H25': 'time_series/{owner}/{feature}/{parameter}/{method}/{processing_code}/{aggregation_statistic}/{frequency_interval}/{utc_offset}/{date}.H25.nc'
                    }

key_patterns = {'ts': 'tethys/{dataset_id}/{site_id}/{from_date}_{to_date}.json.zst',
                'dataset': 'tethys/datasets.json',
                'site': 'tethys/{dataset_id}/sites.json.zst'
                }
...
def update_remote_dataset(s3, datasets, run_date=None):
    """
    ...
    try:
        obj1 = s3.get_object(Bucket=param['remote']['bucket'], Key=ds_key)
    ...
    if rem_ds != datasets:
        ...
        obj2 = s3.put_object(Bucket=param['remote']['bucket'], Key=ds_key, Body=ds_json1, Metadata={'run_date': run_date_key}, ContentType='application/json')
    else:
...
def assign_site_id(geometry):
    ...
    """
    site_id = blake2b(geometry.json().encode(), digest_size=12).hexdigest()
...
# site5 = Site(site_id=site_id, ref='Acton Stream at Hillas Road', virtual_site=False, geometry=geo2, stats=stats1)
