...
def update_remote_sites(s3, dataset_id, site_list, run_date=None):
    """
    ...
    try:
        obj1 = s3.get_object(Bucket=param['remote']['bucket'], Key=site_key)
    ...
    if rem_site != site_list:
        ...
        obj2 = s3.put_object(Bucket=param['remote']['bucket'], Key=site_key, Body=site_json1, Metadata={'run_date': run_date_key}, ContentType='application/json')
    else:
...
def get_qc_hilltop_data(param, ts_local_tz, site_mtype_corrections):
    ...
    try:
        ...
        dataset_list = update_remote_dataset(s3, dataset_list, run_date_key)
        ...
        for ds_id, site_list in sites_dict.items():
            up_sites = update_remote_sites(s3, ds_id, site_list, run_date=run_date)
