...
class TradingPortfolio():
  ...
  def backtest(self, results, min_freq, quote_assets=['BTC', 'USDT', 'BUSD', 'ETH']):
    import matplotlib.pyplot as plt
    position = {}
    quote_substract = {}
    for index, value in results.transpose().items():
      position[value.loc['name']+'|'+ value.symbol + '|' + value.freq] = (value.portfolio.cash == 0) * value.weight
    ...
    addition_usdt = -min(position.USDT.min(), 0)
    if addition_usdt > 0:
        print('WARRN**: additional usdt is required to be margin: ', addition_usdt, 'in btc value')

    ohlcv_usdt = {a:get_all_binance(a+'USDT', min_freq) for a in position.columns if a != 'USDT'}

    all_assets = position.columns
    all_times = position.diff()

    p = position.loc[position.index[(position != position.shift()).abs().sum(axis=1) != 0]]
    ohlcv_usdt_close = pd.DataFrame({name:s.close for name, s in ohlcv_usdt.items()})
    ohlcv_usdt_close = ohlcv_usdt_close.loc[p.index & ohlcv_usdt_close.index]
    p = p.loc[ohlcv_usdt_close.index].fillna(0)

    asset_return = ((ohlcv_usdt_close.pct_change().shift(-1).fillna(0)) * p) - 0.002 * (p - p.shift()).abs()
    asset_return.fillna(0, inplace=True)

    asset_return.cumsum().plot()
    plt.show()

    s = asset_return.sum(axis=1).cumsum() + sum(list(self._margins.values()))
    s.plot()
...
def render_html(signals_df, rebalance_df, rebalance_df_in_btc, order_results, mode):
