...
def Strategy(**default_parameters):
    ...
    class Strategy:
        ...
        def backtest(self, ohlcv, variables=None, filters=None, plot=False, lookback=None, signals=False, side='long', **args):
            ...
            if side == 'short':
                ohlcv_inv = pd.DataFrame()
                ohlcv_inv['close'] = ohlcv_lookback.close / ohlcv_lookback.ohlcv.close.shift(1)
                ohlcv_inv['open'] = ohlcv_lookback['open'] / ohlcv_lookback['close'] * ohlcv_inv['close']
                ohlcv_inv['high'] = ohlcv_lookback['high'] / ohlcv_lookback['close'] * ohlcv_inv['close']
                ohlcv_inv['low'] = ohlcv_lookback['low'] / ohlcv_lookback['close'] * ohlcv_inv['close']
                ohlcv_inv['volume'] = ohlcv_lookback['volume']
