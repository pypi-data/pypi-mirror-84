...
class TradingPortfolio():
  ...
  def backtest(self, results, min_freq, quote_assets=['BTC', 'USDT', 'BUSD', 'ETH'], fee=0.002):
    ...
    for a, w in self._margins.items():
        position[a] += get_asset_price_in_btc(a) * w
        initial_margin_sum_btc += get_asset_price_in_btc(a) * w
    ...
    addition_usdt = -min(position.USDT.min(), 0) / get_asset_price_in_btc('USDT')
    ...
    (asset_return.cumsum() / get_asset_price_in_btc('USDT')).plot()
    plt.show()

    s = (asset_return.sum(axis=1).cumsum() + initial_margin_sum_btc) / get_asset_price_in_btc('USDT')
    s.plot()
