...
from engpy.misc.miscs import simp_var, num, lexpr
...
class Matrix(MatrixObjectClass, Utilities.matrix, StaticStates):
    ...
    def null(self,order):
        ''' Returns Null Matrix'''
        mat = self.empty
        mat.Mat = list(order) + [0 for i in range(order[1]*order[0])]
        return mat
    ...
    def __invert__(self):
        ...
        self._mat = self.Mat
        self.aug()
        emulated = mat.unit(self.order);count = 0
...
class Matrix_(MatrixObjectClass, Utilities.matrix, StaticStates):
    ...
    def null(self,order):
        ''' Returns Null Matrix'''
        mat = self.empty
        mat.Mat = list(order) + [Expr({0: [{'':0}]}) for i in range(order[1]*order[0])]
        return mat
