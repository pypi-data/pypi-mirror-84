        ...
        for i, person_i in enumerate(total_people):
            ...
            for j, person_j in enumerate(total_people):
                time = 0 if person_i['type'] == 'Doctor' and person_j['type'] == 'Doctor' else Helper.calc_time(
                    person_i['location'], person_j['location'])
                self.travel_times[i][j] = time
                total_time = time + person_j.get('appt_duration') or person_i.get('appt_duration') +  self.buffer
                distance_person_i.append(total_time)
