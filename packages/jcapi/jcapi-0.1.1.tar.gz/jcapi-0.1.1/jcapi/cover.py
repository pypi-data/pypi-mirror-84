...
class JcCover:
    ...
    async def get_level(self):
        ...
        """
        value = await self.director.get_item_variable_value(self.item_id, var_name=None, dev_type="cl1")
        return int(value * 10)

    async def get_state(self):
        """Returns the state of a tcq as a boolean (True=on, False=off).0开，1停，2关
        """
        value = await self.director.get_item_variable_value(self.item_id, var_name=None, dev_type="cl")
        if value == "2":
            return False
        else:
            return True
