...
from shyft.dashboard.apps.water_route_app.mock_water_route import generate_hydro_power_system
from shyft.dashboard.apps.water_route_app.basic_water_graph import BasicWaterRouteGraph
...
class WaterRouteApp(AppBase):
    ...
    def get_layout(self, doc: Document, logger: Optional[LoggerBox] = None) -> LayoutDOM:
        ...
        # --------------------------------------------------------------------------------------------------------------
        hps = generate_hydro_power_system()

        # --------------------------------------------------------------------------------------------------------------
        # Using the custom water-route-graph-app to visualize the water course
        # --------------------------------------------------------------------------------------------------------------
        water_route_graph = BasicWaterRouteGraph()
