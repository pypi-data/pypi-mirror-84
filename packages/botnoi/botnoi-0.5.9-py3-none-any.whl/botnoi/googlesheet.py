...
class botnoigsheet():
    def __init__(self,sheeturl,sheetindex=0,credentialjson='credentials.json'):
    ...
    def getcellvalue(self,selectcol,selectvalue,replacecol):
        ...
        return rowpos,colvalpos,value

    def getrow_fromcolname(self,selectcol):
        colpos = self.sheet.find(selectcol,in_row=0).col
        values_list = self.sheet.col_values(colpos)
        return values_list

    def getrow_fromcolindex(self,colindex):
        values_list = self.sheet.col_values(colindex)
        return values_list

    def getcol_fromrowindex(self,rowindex):
