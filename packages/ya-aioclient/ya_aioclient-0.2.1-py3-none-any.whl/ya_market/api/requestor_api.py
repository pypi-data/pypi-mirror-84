...
class RequestorApi(object):
    ...
    def cancel_agreement(self, agreement_id, **kwargs):  # noqa: E501
        ...
        Causes:   - the awaiting `approveAgreement` on Provider side to return with `Cancelled` response.   - the awaiting `waitForApproval` local call to return with `Cancelled` response.   # noqa: E501
    ...
    def confirm_agreement(self, agreement_id, **kwargs):  # noqa: E501
        """Sends Agreement proposal to the Provider.  # noqa: E501
    ...
    def wait_for_approval(self, agreement_id, **kwargs):  # noqa: E501
        ...
        This is a blocking operation. The call may be aborted by Requestor caller code. After the call is aborted, another `waitForApproval` call can be raised on the same Agreement Id. It returns one of the following options: * `Approved` - Indicates that the Agreement has been approved by the Provider.   - The Provider is now ready to accept a request to start an Activity     as described in the negotiated agreement.   - The Providers’s corresponding `approveAgreement` call returns `Approved` after     this on the Provider side.  * `Rejected` - Indicates that the Provider has called `rejectAgreement`,   which effectively stops the Agreement handshake. The Requestor may attempt   to return to the Negotiation phase by sending a new Proposal.  * `Cancelled` - Indicates that the Requestor himself has called  `cancelAgreement`, which effectively stops the Agreement handshake.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
