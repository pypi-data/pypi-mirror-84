...
class Diagnostic(object):
    ...
    def children(self):
        class ChildDiagnosticsIterator(object):
            ...
            def __len__(self):
                return int(conf.lib.clang_getNumDiagnosticsInSet(
                    self.diag_set))
...
def _upper_to_standard(string: str) -> str:
    res = ""
    state = 0
    for s in string:
        if state == 0:
...
def cursor_dumps(cursor: "Cursor",
                 indent: int = 2,
                 start_indent=0,
...
class Cursor(Structure):
    ...
    def get_template_cursor_kind(self):
        ...
        determine the cursor kind of the specializations would be 
        generated by instantiating the template.
        This routine can be used to determine what flavor of function template, 
        class template, or class template partial specialization is stored in 
        the cursor. For example, it can describe whether a class template cursor 
        is declared with "struct", "class" or "union"."""
    ...
    def get_specializations(self) -> List["Cursor"]:
        # TODO handle function template
        specs = []
        if self.kind == CursorKind.CLASS_TEMPLATE:
            for child in self.get_children():
                if child.kind == CursorKind.CLASS_TEMPLATE_SPECIALIZATION:
                    specs.append(child)
        elif self.kind == CursorKind.CLASS_TEMPLATE_PARTIAL_SPECIALIZATION:
            cls_template = self.get_specialized_cursor_template()
            assert cls_template.kind == CursorKind.CLASS_TEMPLATE
            cls_temp_specs = cls_template.get_specializations()
            for spec in cls_temp_specs:
    ...
    def evaluate(self):
        return EvaluateResult(conf.lib.clang_Cursor_Evaluate(self))
...
class Type(Structure):
    ...
    ArrayTypes = [
        ...
        TypeKind.VARIABLEARRAY, TypeKind.DEPENDENTSIZEDARRAY, TypeKind.VECTOR
    ]
    RefTypes = [TypeKind.LVALUEREFERENCE, TypeKind.RVALUEREFERENCE]
    ...
    def _get_plain_type(self, nested_type: "Type") -> "Type":
        if nested_type.kind in self.ArrayTypes:
            return self._get_plain_type(nested_type.get_array_element_type())
        elif nested_type.kind in self.PointerTypes:
            return self._get_plain_type(nested_type.get_pointee())
        elif nested_type.kind in self.RefTypes:
            return self._get_plain_type(nested_type.get_pointee())
        else:
            return nested_type

    def get_plain_type(self) -> "Type":
        """remove all pointer/ref/array of a type
        """
...
class EvaluateResult(ClangObject):
    def __init__(self, obj):
    ...
    def __repr__(self):
        if self.valid:
            return f"EvalResult[{self.value}]"
        else:
            return f"EvalResult[Invalid]"
    ...
    @property
    def kind(self):
        return EvalResultKind(conf.lib.clang_EvalResult_getKind(self))

    @property
    def value(self):
        if self.valid:
            kind = self.kind
            if kind == EvalResultKind.Int:
                return conf.lib.clang_EvalResult_getAsLongLong(self)
            elif kind == EvalResultKind.Float:
                return conf.lib.clang_EvalResult_getAsDouble(self)
            elif kind == EvalResultKind.CFStr:
                raise NotImplementedError
            elif kind == EvalResultKind.StrLiteral:
                return conf.lib.clang_EvalResult_getAsStr(self)
            elif kind == EvalResultKind.ObjCStrLiteral:
                raise NotImplementedError
            elif kind == EvalResultKind.Other:
                raise NotImplementedError
            elif kind == EvalResultKind.UnExposed:
                raise NotImplementedError
            else:
                raise NotImplementedError
        return None
...
functionList = [
    ...
    ("clang_codeCompleteAt",
     [TranslationUnit, c_interop_string, c_int, c_int, c_void_p, c_int,
      c_int], POINTER(CCRStructure)),
    ("clang_codeCompleteGetDiagnostic", [CodeCompletionResults,
                                         c_int], Diagnostic),
    ...
    ("clang_Cursor_Evaluate", [Cursor], c_object_p),
    ("clang_EvalResult_getKind", [EvaluateResult], c_uint),
    ("clang_EvalResult_getAsDouble", [EvaluateResult], c_double),
    ("clang_EvalResult_getAsLongLong", [EvaluateResult], c_longlong),
    ("clang_EvalResult_getAsStr", [EvaluateResult], c_char_p),
    ("clang_formatDiagnostic", [Diagnostic,
                                c_uint], _CXString, _CXString.from_result),
    ...
    ("clang_getCompletionBriefComment", [c_void_p], _CXString,
     _CXString.from_result),
    ("clang_getCompletionChunkCompletionString", [c_void_p,
                                                  c_int], c_object_p),
    ...
    ("clang_getCursorReferenceNameRange", [Cursor, c_uint,
                                           c_uint], SourceRange),
    ...
    ("clang_getLocation", [TranslationUnit, File, c_uint,
                           c_uint], SourceLocation),
    ("clang_getLocationForOffset", [TranslationUnit, File,
                                    c_uint], SourceLocation),
    ...
    ("clang_getOverloadedDecl", [Cursor,
                                 c_uint], Cursor, Cursor.from_cursor_result),
    ...
    ("clang_getSpecializedCursorTemplate", [Cursor], Cursor,
     Cursor.from_cursor_result),
    ("clang_getTemplateCursorKind", [Cursor], CursorKind.from_id),
    ...
    ("clang_getTokenSpelling", [TranslationUnit,
                                Token], _CXString, _CXString.from_result),
    ...
    ("clang_reparseTranslationUnit", [TranslationUnit, c_int, c_void_p,
                                      c_int], c_int),
    ("clang_saveTranslationUnit", [TranslationUnit, c_interop_string,
                                   c_uint], c_int),
    ("clang_tokenize",
     [TranslationUnit, SourceRange,
      POINTER(POINTER(Token)),
      POINTER(c_uint)]),
    ("clang_visitChildren", [Cursor, callbacks['cursor_visit'],
                             py_object], c_uint),
    ...
    ("clang_Cursor_getTemplateArgumentUnsignedValue", [Cursor,
                                                       c_uint], c_ulonglong),
    ...
    ("clang_Type_visitFields", [Type, callbacks['fields_visit'],
                                py_object], c_uint),
]
