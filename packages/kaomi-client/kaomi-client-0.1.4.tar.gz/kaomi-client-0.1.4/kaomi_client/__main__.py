...
def get_arg_parser():
    ...
    p.add_argument('-is', '--include-subdirs',                       dest='include_subdirs',                      action='store_true', required=False, default=False,  help="specifies whether to include subdirectories when using regex deletion (default: %(default)s)")
    p.add_argument('-i', '--inverted',                               dest='inverted',                             action='store_true', required=False, default=False,  help="specifies whether to delete the not matching files/folders instead of the matching ones when using regex deletion (default: %(default)s)")
...
if args.action is not None:
    ...
    try:
        # noinspection PyUnboundLocalVariable
        server_resp = kaomi_client.delete_by_regex(path=base_path, regex=regex, inverted=args.inverted, node_type=args.node_type, include_subdirs=args.include_subdirs)
    except ConnectionError as e:
