...
def resnet50_3D(inputs, n=2, include_fc_layer=False):
    ...
    conv_block = lambda filters1, filters2, x, strides: relu(
        add(conv_sc(filters2, x, strides), conv_3(filters2, conv_2(filters1, conv_1(filters1, x, strides)))))
    # identity blocks
    ...
    # --- ResNet-50 backbone
    # stage 1 c2 1/4
    res1 = max_pool(zeropad(relu(norm(conv7(zeropad(inputs, (0, 3, 3)), 64, strides=(1, 2, 2)))), (0, 1, 1)),
                    (1, 3, 3),
                    strides=(1, 2, 2))
    # stage 2 c2 1/4
    res2 = layers.Lambda(lambda x: x, name='c2-output')(
        identity_block(64, 256, identity_block(64, 256, conv_block(64, 256, res1, strides=1))))
    # stage 3 c3 1/8
    res3 = layers.Lambda(lambda x: x, name='c3-output')(
        identity_block(128, 512, identity_block(128, 512, identity_block(128, 512, conv_block(128, 512, res2,
                                                                                              strides=(1, 2, 2))))))
    # stage 4 c4 1/16
    res4 = layers.Lambda(lambda x: x, name='c4-output')(
        identity_block(256, 1024, identity_block(256, 1024, identity_block(256, 1024, identity_block(256, 1024,
                                                                                                     identity_block(
                                                                                                         256, 1024,
                                                                                                         conv_block(
                                                                                                             256,
    ...
    res5 = layers.Lambda(lambda x: x, name='c5-output')(
        identity_block(512, 2048, identity_block(512, 2048, conv_block(512, 2048, res4, strides=(1, 2, 2)))))
...
def feature_pyramid_3D(inputs):
    ...
    p5 = conv1(inputs[2], 256, strides=1)
    fp4 = fp_block(p5, inputs[1])
    p4 = conv3(fp4, 256, strides=1)
    fp3 = fp_block(fp4, inputs[0])
    p3 = conv3(fp3, 256, strides=1)
    p6 = conv3(p5, 256, strides=(1, 2, 2))
    p7 = conv3(relu(p6), 256, strides=(1, 2, 2))
    return [p3,p4,p5,p6,p7]


def class_and_reg_subnets(feature_pyramid, K, A, shared_weights=False):
    if shared_weights:
        ...
        for i in feature_pyramid:
            class_models.append(classificaiton_head(K, A))
            reg_models.append(regression_head(A))
...
def retinanet_resnet_2(inputs, K, A):
    ...
    # --- classification head
    class_subnet3 = classificaiton_head(K, A)
    class_subnet4 = classificaiton_head(K, A)
    class_subnet5 = classificaiton_head(K, A)
    # --- regression head
