...
def load_model_and_datas(tokenizer, pretrained, device, model_arg, input_arg):
    ...
    train_ds_maxlen = 0
    test_ds_maxlen = 0
    for model_class_name, train_file, test_file in zip_longest(input_arg.model, input_arg.train, input_arg.test,
                                                               fillvalue=""):
        # get model class
        model_class = load_model_class(model_class_name)

        # load dataset
        train_ds = get_dataset(model_arg, model_class, input_arg, train_file)
        test_ds = get_dataset(model_arg, model_class, input_arg, test_file)
...
def main():
    ...
    input_arg, model_arg = parser.parse_known_args()
    model_arg = {k.replace("--", ""): v for k, v in zip(model_arg[:-1:2], model_arg[1::2])}
    nlp2.get_dir_with_notexist_create(input_arg.savedir)
    logger = Logger(savedir=input_arg.savedir, tensorboard=input_arg.tensorboard)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    nlp2.set_seed(input_arg.seed)

    logger.write_log("TRAIN PARAMETER")
    logger.write_log("=======================")
    [logger.write_log(var, ':', vars(input_arg)[var]) for var in vars(input_arg)]
    logger.write_log("=======================")
    ...
    if input_arg.add_tokens:
        ...
        add_tokens = tok.get_freqK_unk_token(tokenizer, input_arg.train + input_arg.test, input_arg.add_tokens)
    ...
    start_epoch = 1
    if input_arg.resume:
        logger.write_log("Loading back:", input_arg.resume)
    ...
    # train/eval loop
    logger.write_log("training batch : " + str(input_arg.batch * input_arg.grad_accum))
