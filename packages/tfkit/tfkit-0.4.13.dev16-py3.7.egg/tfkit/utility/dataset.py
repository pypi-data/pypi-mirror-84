...
def get_dataset(model_arg, model_class, dataset_arg, file_path):
    panel = nlp2.Panel()
    inputted_arg = {"maxlen": dataset_arg.maxlen, 'handle_exceed': dataset_arg.handle_exceed}
    inputted_arg.update(model_arg)
    all_arg = nlp2.function_get_all_arg_with_value(model_class.preprocessing_data)
    if dataset_arg.panel:
    ...
    ds = LoadDataset(fpath=file_path, pretrained_config=dataset_arg.config,
                     get_data_from_file=model_class.get_data_from_file,
                     preprocessing_data=model_class.preprocessing_data,
                     cache=dataset_arg.cache,
                     input_arg=inputted_arg)
    return ds
