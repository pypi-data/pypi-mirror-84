...
def _cli_main(parser: argparse.ArgumentParser, args_list: List[str]):
    ...
    if with_storage:
        ...
        if storage_connection is None or storage_connection == "":
            print("Storage connection string must be provided in the configuration file or as --storage-connection option.")
            parser.exit(_DEFAULT_ERROR)
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--storage-connection', 
                        help='Storage connection string. Used with cache and list stages when is not in the configuration file.',
