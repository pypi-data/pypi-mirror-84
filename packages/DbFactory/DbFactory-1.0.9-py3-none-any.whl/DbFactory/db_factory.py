...
class DbFactory(with_metaclass(Singleton), IdleTips):
    ...
    def __init__(
            ...
            singleton_num='0',  # 非必需 单例编号，如果开启单例模式，可以使用编号开启多个单例
            retry_times=10,  # 非必需 错误重试次数
            singleton_sign=None,  # 非必需 'any string' 单例名称标记，用来区分单例
        ...
        # db_factory
        self.singleton_sign = singleton_sign or ''  # 单例标记
    ...
    def __init_log(self):
        ...
        if not self._log:
            ...
            if self._debug:     # todo 能够改变其他模块的日志等级？因为可能出现日志不规范
                self._log.basicConfig(level=logging.DEBUG)
            else:
                self._log.basicConfig(level=logging.INFO)
    ...
    def origin_client(self):
        ...
        if self._db_type in {"MYSQL", "KAFKA"}:
            self._log.info('use origin_client')  # todo 确定调试日志级别
            return self.client.client__.origin_client(self.singleton_sign)
        else:
            self._log.warning('该类型，直接反射出db 暂时没有 异常处理加成')
