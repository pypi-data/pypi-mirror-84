...
try:
    ...
    if not (user_python_version.major >= MIN_PYTHON_MAJOR and user_python_version.minor >= MIN_PYTHON_MINOR):
        logging.warning(f"{Style.BRIGHT}{Fore.RED}\tIt is highly advised that you use Python {MIN_PYTHON_MAJOR}.{MIN_PYTHON_MINOR} or greater...\n"
        ...
        logging.warning(f"\tPlease decide if you want to proceed with running as is or not...\n")
    ...
    logging.error(f'{Style.BRIGHT}{Fore.RED}\tUnable to get the version of Python you are using...\n'
...
try:
    ...
    if latest_version > version.StrictVersion(__version__):
        if version.StrictVersion(__version__) < version.StrictVersion('0.1.0'):
            logging.warning(f'{Style.BRIGHT}{Fore.GREEN}Using DEVELOPER version (deVer-{__version__})...{Style.RESET_ALL}')
            update_notification_sleep_time = 0
        if update_notification_sleep_time > 0:
            logging.warning(f"{Style.BRIGHT}{Fore.GREEN}\tThere is a newer version of this library available ({latest_version}), while your version is {__version__}...\n"
        ...
        while update_notification_sleep_time >= 0:
            sleep(1)
            end_char = "\r" if update_notification_sleep_time > 0 else "\n"
            logging.warning(f'{Style.BRIGHT}{Fore.GREEN}Your code will resume in {update_notification_sleep_time} secs...{Style.RESET_ALL}')
            update_notification_sleep_time = update_notification_sleep_time - 1
except Exception as e:
    logging.error(f'{Style.BRIGHT}{Fore.RED}\tUnable to get latest version from PyPi...\n'
                  f'\tPlease report this error to truedata (https://github.com/paritoshjchandran/truedata/issues/new)...\n'
                  f'\tAlong with the following information - {e}...{Style.RESET_ALL}')
