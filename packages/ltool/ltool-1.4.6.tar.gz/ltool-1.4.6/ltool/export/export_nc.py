...
def nc_name(metadata, prod_id, wave):
    ...
    st_id = metadata['station_ID'].zfill(7)
    
    wave = wave.zfill(4)
    
    start_t = parse_date.parse(metadata[0]['measurement_start_datetime'])\
        .strftime('%Y%m%d%H%M')
    stop_t = parse_date.parse(metadata[0]['measurement_stop_datetime'])\
        .strftime('%Y%m%d%H%M')
