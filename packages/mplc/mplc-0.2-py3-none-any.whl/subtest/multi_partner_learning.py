...
class MultiPartnerLearning:
    ...
    def compute_test_score_for_single_partner(self, partner):
        ...
        start = timer()
        logger.info(f"## Training and evaluating model on partner with partner_id #{partner.id}")
    ...
    def prepare_aggregation_weights(self):
        ...
        if self.aggregation_weighting == "uniform":
            ...
            raise NameError("The aggregation value [" + self.aggregation_weighting + "] is not recognized.")
    ...
    def log_collaborative_round_partner_result(self, partner, partner_index, validation_score):
        ...
        partner_id_str = f"Partner partner_id #{partner.id} ({partner_index}/{self.partners_count - 1})"
