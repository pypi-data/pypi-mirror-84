...
def train_early_stop(model, loader, criterion, optimizer, scheduler, I=10):
    ...
    cc_timer = cc_clock.AllUnitsTimer()
    ...
    time_training = cc_timer.get_all_unit_time()

    print('Training complete in {d:s} days {h:s} hours {m:s} minutes {s:s} seconds'.format(
        d=str(time_training.days), h=str(time_training.hours),
        m=str(time_training.minutes), s=str(time_training.seconds)))
...
def load_model(name_model, K=None, pretrained=False, device=None):
    ...
    dict_locals = {'torchvision': torchvision, 'pretrained': pretrained}
