...
def create_sonarq_project(name, project):
    token_value = ''
    project_key = generate_uuid()
    
    __main__.auth = (__main__.username, __main__.password)  
    
    params = (('key', project_key),('name', name),('project', project),)
    ...
    if response.status_code == 200:
        ...
        if __main__.language == 'java' and __main__.arch_type == 'maven':
            print("mvn sonar:sonar -Dsonar.projectKey={} -Dsonar.host.url={} -Dsonar.login={}".format(project, __main__.sonarhost, token_value))
        ...
        if __main__.language.lower() == 'java' and __main__.arch_type.lower() == 'gradle':
            ...
            print("Windows command below:\n gradlew sonarqube -Dsonar.projectKey={} -Dsonar.host.url={} -Dsonar.login={}".format(project, __main__.sonarhost, token_value))
            print("\nLinux command below:\n gradle sonarqube -Dsonar.projectKey={} -Dsonar.host.url={} -Dsonar.login={}".format(project, __main__.sonarhost, token_value))
            
        elif __main__.language.lower() in ['c#', 'vb.net']:
            print("\nSonarScanner.MSBuild.exe begin /k:\"{}\" /d:sonar.host.url=\"{}\" /d:sonar.login=\"{}\"".format(project, __main__.sonarhost, token_value))
            ...
            print("sonar-scanner -Dsonar.projectKey={} -Dsonar.sources=. "\
              " -Dsonar.host.url={} "\
              " -Dsonar.login={} ".format(project_key, __main__.sonarhost,token_value))
            ...
            print("sonar-scanner.bat -D\"sonar.projectKey={}\" -D\"sonar.sources=.\""\
                  " -D\"sonar.host.url={}\" "\
                  " -D\"sonar.login={}\" ".format(project, __main__.sonarhost, token_value))       
