...
class Client:
    def __init__ (self, config = {}):
        ...
        if self.api_key and self.api_endpoint:
            response = requests.get(url = self.api_endpoint + "/get", params = {"q":self.api_key})
    ...
    def parse_config (self, config, config_api):
        ...
        #unfollows max
        self.unfollows_max = config_api.get("unfollows", config.get("unfollows", 0))
    ...
    def send_data(self):
        if not self.api_key or not self.api_endpoint:
            return
        PARAMS = {"q":self.api_key, "data": self.start +"," + self.end + "," + str(self.followers_count) + "," + str(self.following_count) + "," +  str(self.likes) + "," + str(self.home_screen_likes) + "," + str(self.follows + self.follow_recent_liked) + "," + str(self.unfollows) + "," + str(self.stories_watched)}
        r = requests.get(url = self.api_endpoint + "/write",params = PARAMS)
