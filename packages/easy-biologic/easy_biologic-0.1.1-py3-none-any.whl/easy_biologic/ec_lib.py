...
def create_parameters( params, index = 0 ):
    ...
    :param params: A dictionary of parameters, with keys as
        the parameter name and values as the parameter value.
    :param index: The index to apply to all the parameters.
        [Default: 0]
    :returns: A EccParams structures.
    """
    param_list = []
    for name, value in params.items():
        param = create_parameter( name, value, index )
...
def update_parameters( idn, ch, technique, params, index = 0 ):
    """
...
def get_data( idn, ch ):
    ...
    :param idn: Device identifier.
    :param ch: Channel.
    :returns: A tuple of ( data_info, data, current_values ) where
...
def technique_file( technique, device = None ):
    ...
    :param device: Kind fo device. [Default: None]
