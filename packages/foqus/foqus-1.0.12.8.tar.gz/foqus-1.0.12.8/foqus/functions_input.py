...
def process_customer_stream_from_json(customer_name, customer_type, project_name):
    ...
    if parameters.get('algorithm', 'vectors') == 'vectors':
        ...
        if int(response) != 0:
            ...
            if len(folders) > 0:
                try:
                    ...
                    logger.info("First time training for project %s client %s with type %s" % (project_name,
                                                                                               customer_name,
                                                                                               customer_type))
                    train_details = {}
        ...
        logger.info("Training_Similars not launched client %s type %s parameter is %s"
                    % (customer_name, customer_type, parameters.get('algorithm')))
