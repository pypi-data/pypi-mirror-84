...
#     def get_backup(self, reference: str, pipeline_name: str = None) -> model.AbstractModel:
#         cls = self._get_class(reference)
#         dataobj = self._get(cls.source_table)
#         modelobj = cls( \
#             reference=reference, \
#             dataobject=dataobj,\
#             api=self.api,\
...
#         return modelobj
    ...
    def get(self, reference: str, pipeline_name: str = None) -> model.AbstractModel:
        cls = self._get_class(reference)
        dataobj_list = [ getattr(self, '_get_'+source.source_type)(source.source_table)  for source in cls.pipeline._datasources ]
...
#     def get_staging(self, *, reference: str, staging_sources: List[str]=None) -> model.AbstractModel:
#         cls = self._get_class(reference)

#         df_list = [self._load(s) for s in staging_sources]
#         modelobj = cls.from_join(*df_list, \
#                                  reference=reference, \
#                                  api=self.api, \
#                                  data_path=self.data_path)
#         if modelobj:
