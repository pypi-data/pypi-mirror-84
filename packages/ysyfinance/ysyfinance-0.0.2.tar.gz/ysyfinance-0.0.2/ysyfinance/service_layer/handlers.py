...
__all__ = ['__all__', 'etl_assignment', 'run_etljob', 'EVENT_HANDLERS', 'COMMAND_HANDLERS']
...
# Cell
__all__=['etl_assignment', 'run_etljob']

# Cell
def etl_assignment(
    cmd: commands.EtlAssignment, pd_uow: unit_of_work.AbstractUnitOfWork
):
    with pd_uow:
        assignment=pd_uow.repo.get(reference=cmd.reference)
        pd_uow.repo.session.save('assignment_precalc', assignment)
...
def run_etljob(
    ...
    with syn_uow:
        ...
        for pname in pipeline_names:
            ...
            model = model_ori.run_pipeline(pipeline_name=pname)
...
#     commands.EtlRbac: etl_rbac,
#     'etl_uvgl': etl_uvgeneralledger,
