...
__all__ = ['__all__', 'AbstractUnitOfWork', 'spark_sessionmaker', 'DEFAULT_SPARK_SESSION_FACTORY',
           'pandas_sessionmaker', 'DEFAULT_PD_SESSION_FACTORY', 'SparkUnitOfWork', 'PandasUnitOfWork', 'get_syn_uow']
...
__all__=['AbstractUnitOfWork','spark_sessionmaker','SparkUnitOfWork',\
         'PandasUnitOfWork','pandas_sessionmaker','get_syn_uow']
...
class SparkUnitOfWork(AbstractUnitOfWork):
    def __init__(self, session_factory=DEFAULT_SPARK_SESSION_FACTORY):
...
# Cell
def get_syn_uow(spark_sessionmaker) -> (SparkSession, SparkUnitOfWork):
    # use below to debug
#     db.set_trace()
    spark_session_factory=spark_sessionmaker()
    syn_uow=SparkUnitOfWork(spark_session_factory)
    return syn_uow