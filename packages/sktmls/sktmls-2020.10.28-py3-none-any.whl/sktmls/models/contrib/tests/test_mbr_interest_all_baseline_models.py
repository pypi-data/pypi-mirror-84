...
class TestMbrInterestAllBaselineModel:
    def test_000_mbr_valid_baseline_1(self):
        ...
        assert item["props"]["context_id"] == "context_preference"
        assert item["props"]["context_nm"] == "고객선호"
    ...
    def test_001_mbr_valid_baseline_2(self):
        ...
        assert item1["props"]["context_id"] == "context_preference"
        assert item1["props"]["context_nm"] == "고객선호"
        assert float(item1["props"]["score"]) > 1.0

        item2 = results["items"][1]
        assert item2["id"] in ["b1", "b2", "3", "4"]
        assert item2["type"] == "test_type"
        assert item2["props"]["context_id"] == "context_preference"
        assert item2["props"]["context_nm"] == "고객선호"
    ...
    def test_002_mbr_valid_baseline_2_prefer_none(self):
        ...
        assert item1["type"] == "test_type"
        assert item1["props"]["context_id"] == "context_default"
        assert item1["props"]["context_nm"] == "#"
        assert float(item1["props"]["score"]) < 1.0

        item2 = results["items"][1]
        assert item2["id"] in ["a1", "a2", "a3", "a4"]
        assert item2["type"] == "test_type"
        assert item2["props"]["context_id"] == "context_default"
        assert item2["props"]["context_nm"] == "#"
        assert float(item2["props"]["score"]) < 1.0
