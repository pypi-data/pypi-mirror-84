    ...
    def generate_auto_input_params_file():
        ...
        for api, info in f1.items():
            ...
            for method, method_info in info.items():
                    ...
                    if k in random_data_set["header"]:
                        if not header_already_added:
                            ...
                            if method not in input_params[api]["200"]:
                                input_params[api]["200"][method] = [one_data_block]
                            else:
                                input_params[api]["200"][method].append(one_data_block)
                            ...
                            for L in range(0, len_to_process ):
                                for _ in combinations(v, L):
                                    if not _:
                                        continue
                                    _ = list(_) #",".join(_)
                                    some_data_set = deepcopy(random_data_set)
                ...
                for _ in input_params[api]["200"][method]:
                    if "header" in _:
                       del _["header"]

                if method not in input_params[api]["401"]:
                    rand_choice = random.choice(input_params[api]["200"][method])
