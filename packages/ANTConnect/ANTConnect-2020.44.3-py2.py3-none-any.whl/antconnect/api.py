...
class API:
    ...
    def records_search(self, project_id: str, table_id: str, searchFields = list, searchPhrase: str='', searchExact: str='', searchRange: list = [], limit: int = 0, offset: int = 0) -> dict:
        """ Get reords of table """
        path = 'search'
        if searchPhrase != '':
            search = {
        ...
        record_data = self._make_api_request(path, 'POST', {
            "project": {"id": project_id},
            "table": {"id": table_id},
            "search": search,
