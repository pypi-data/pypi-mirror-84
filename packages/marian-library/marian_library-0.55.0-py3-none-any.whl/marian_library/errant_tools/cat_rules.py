...
def getEditInfo(toks, tag_map):
    ...
    for tok in toks:
        str.append(tok.text)
        pos.append(
            tag_map.get(tok.tag_, "X")
        )  # if we can't find the tag, don't die, just use X, means Unknown
        dep.append(tok.dep_)
    return str, pos, dep
...
def getTwoSidedType(orig_toks, cor_toks, gb_spell, tag_map, nlp, stemmer):
    ...
    if len(orig_str) == len(cor_str) == 1:
        ...
        # Contraction. Rule must come after possessive.
        if (
            orig_str[0].lower() in conts or cor_str[0].lower() in conts
        ...
        if (
            ...
            and orig_pos[0] in open_tags
            and cor_pos[0] in open_tags
        ):
    ...
    if (orig_pos == ["NOUN", "PART"] or cor_pos == ["NOUN", "PART"]) and sameLemma(
    ...
    if (
        ...
        and sameLemma(orig_toks[-1], cor_toks[-1], nlp)
        and len(orig_str) <= 2
        and len(cor_str) <= 2
    ):
        return "ADJ:FORM"
