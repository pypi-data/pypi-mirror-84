...
import marian_library.errant_tools.rdlextra as DL
...
def get_edits(source, target, edits):
    ...
    for op, group in groupby(
        ...
        # Further processing required
            ...
            out_edits.extend(process_edits(source, target, group))
    return out_edits
...
def process_edits(source, target, edits):
    ...
    for start, end in combos:
        ...
        if s[-1].tag_ == "POS" or t[-1].tag_ == "POS":
            return (
                process_edits(source, target, edits[: end - 1])
                + merge_edits(edits[end - 1 : end + 1])
                + process_edits(source, target, edits[end + 1 :])
            )
        # Case changes
        if s[-1].lower_ == t[-1].lower_:
            ...
            if start == 0 and (
                ...
                return merge_edits(edits[start : end + 1]) + process_edits(
        ...
        if s_str == t_str:
            return (
                process_edits(source, target, edits[:start])
                + merge_edits(edits[start : end + 1])
                + process_edits(source, target, edits[end + 1 :])
            )
        ...
        if end - start < 2:
            ...
            if len(s) == len(t) == 2:
                ...
                ) + process_edits(source, target, edits[start + 1 :])
        ...
        if not pos_set.isdisjoint(CONTENT_POS):
            content = True
...
def getAutoAlignedEdits(orig, cor, args):
    ...
    if args.lev:
        alignments = DL.WagnerFischer(
            orig_toks,
            cor_toks,
            orig,
            cor,
            substitution=levSubstitution,
            transposition=levTransposition,
        )
    # Otherwise, use linguistically enhanced Damerau-Levenshtein
    else:
        alignments = DL.WagnerFischer(
            orig_toks, cor_toks, orig, cor, substitution=token_substitution
        )
