...
class R3sponse(object):
	...
	def get_request_parameter(self, request, identifier):
		response = self.__default_response__()
		variable = request.GET.get(identifier)
		if variable in ["", None]:
			return variable, self.error_response(f"Define parameter: [{identifier}].")
		return variable, self.success_response(f"Succesfully retrieved request parameter [{name}].")
	...
	# check parameters.
	def check_parameter(parameter=None, name="parameter", empty_value=None):
		response = self.__default_response__()
	...
	def check_parameters(parameters={"parameter":None}, empty_value=None):
		response = self.__default_response__()
