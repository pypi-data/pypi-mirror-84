		...
		if isinstance(arguments, str): return argument_present(argument)
			...
			for argument in arguments:
				if argument_present(argument):
					return True
			return False
	def get_argument(self, argument, required=True, index=1, return_format="string", empty=None):
		return get_argument(argument, required=required, index=index, return_format=return_format, empty=empty)
