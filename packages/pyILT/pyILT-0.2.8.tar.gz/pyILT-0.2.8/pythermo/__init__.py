...
from .proplist import prop2abr, abr2prop
...
def query(comp='', numOfComp=0, year='', author='', keywords='', prop=''):
    ...
    print("params=",params)
    #JOAO
    r1 = requests.get(searchUrl, params=params)
    resDict = r1.json()
    if len(resDict['errors']) > 0:
        e = " *** ".join(resDict['errors'])
        #JOAO
        print("len(resDict['errors']) > 0")
        #JOAO
        raise queryError(e)
