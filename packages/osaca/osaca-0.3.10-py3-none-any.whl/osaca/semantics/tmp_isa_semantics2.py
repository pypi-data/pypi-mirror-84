...
class ISASemantics(object):
    ...
    def assign_src_dst(self, instruction_form):
        ...
        if self.has_load(instruction_form):
            instruction_form['flags'] += [INSTR_FLAGS.HAS_LD]
        if self.has_store(instruction_form):
    ...
    @staticmethod
    def has_store(instruction_form):
    ...
    @staticmethod
    def has_indexing(instruction_form):
        """Check for pre- or post-indexing in AArch64 memory addressing"""
        for operand in instruction_form['operands']:
            if 'memory' in operand and ('post_indexed' in operand or 'pre_indexed' in operand):
                return True
        return False

    @staticmethod
    def get_indexing_operands(instruction_form):
        for operand in instruction_form['operands']:
            if 'memory' in operand and 'post_indexed' in operand['memory']:
                # post-indexed
                return 
                raise NotImplementedError
            if 'memory' in operand and 'pre_indexed' in operand['memory']:
                # pre-indexed
                raise NotImplementedError
