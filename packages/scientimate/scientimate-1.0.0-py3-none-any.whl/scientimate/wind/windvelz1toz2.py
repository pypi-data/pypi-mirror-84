def windvelz1toz2(Uz1, z1, z2=10, alpha=1/7, z0=0.005, alphaCh=0.0145):
    """
    .. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
    .. +                                                                        +
    .. + ScientiMate                                                            +
    ...
    Uz2LogLaw=(Ustar/0.41)*np.log(z2/z0) #Calculating wind velocity at z2 m height 
    ...
    for i in range(0,len(Uz1),1):
        fun = lambda x : Uz1[i]-(x/0.41)*np.log(z1/(alphaCh*x**2/9.81)) #Logarithmic velocity profile, U=(Ustar/0.41)*ln(z/z0)
        
        UstarCh[i]=sp.optimize.fsolve(fun,Uz1[i]) #Wave number

    z0Ch=alphaCh*UstarCh**2/9.81 #Charnock (1955) relationship: z0=alphaCh*(Ustar^2/g)
    
    Uz2LogLawCh=UstarCh/0.41*np.log(z2/z0Ch) #Calculating wind velocity at z2 m height 

    #--------------------------------------------------------------------------
    #Outputs
    return Uz2PawLaw, Uz2LogLaw, Uz2LogLawCh
