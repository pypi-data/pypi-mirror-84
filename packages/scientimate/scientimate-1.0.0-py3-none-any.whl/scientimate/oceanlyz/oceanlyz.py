class oceanlyz:
    ...
    def oceanlyzecalcwave(self):
        ...
        if np.sum(np.isnan(d))!=0:
            ...
            warning.warn('Input file contains NaN value(s).')
            warning.warn('NaN value(s) are replaced by linearly interpolated value(s).')
            warning.warn('Oceanlyz contniues with modified data.')
        ...
        if np.sum(np.isinf(d))!=0:
            ...
            warning.warn('Input file contains Inf value(s).')
            warning.warn('Inf value(s) are replaced by linearly interpolated value(s).')
            warning.warn('Oceanlyz contniues with modified data.')
        ...
        if np.sum(d==0)!=0:
            warning.warn('Input file contains Zero value(s), Oceanlyz continues with current data.')
        ...
        for i in range(0,self.n_burst,1):
            ...
            if self.InputType=='waterlevel':
                ...
                h=np.mean(input_data/(1000*9.81))+self.heightfrombed #Calculating mean water depth from pressure data

            if h<=0:
                warning.warn('Mean water depth is Zero or negative, Oceanlyz contniues with mean water depth=0.001 m.')
                h=0.001
