...
class TorchEstimator(Estimator):
    ...
    def export_model(self, opset_version=8, batch=1, verbose=False):
        """
    ...
    def backward_optim(self, train_dataset=None, loss_fn=None, optimizer=None, scheduler=None):
        ...
        for epoch in range(*self.cfg.epoch_range[:2]):
            ...
            for i, sample in enumerate(train_loader, 0):
                ...
                assert isinstance(loss_dict, dict)  # loss = {"loss": loss, "conf_loss": conf_loss}
                ...
                if i % self.cfg.loss_avg_step == self.cfg.loss_avg_step - 1:
                    ...
                    LOG_INFO(
                        "epoch:{} step:{}/{} remain:{:.0f}s {} lr:{}".format(epoch + 1, step, steps_per_epoch, remain,
                                                                             " ".join(loss_msg), lr))
