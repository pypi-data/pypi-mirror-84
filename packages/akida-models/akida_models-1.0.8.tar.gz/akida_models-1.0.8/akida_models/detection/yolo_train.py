...
def compile_model(model_path, anchors, grid_size, tune, freeze, num_classes,
                  batch_size):
    nb_box = len(anchors)
...
def train(model_path, train_data, valid_data, anchors, grid_size, labels,
          obj_threshold, nms_threshold, epochs, tune, freeze, batch_size):
    TRAIN_TIMES = 10

    # Compile model
    model = compile_model(model_path, anchors, grid_size, tune, freeze,
...
def evaluate(model_path, valid_data, anchors, grid_size, labels, obj_threshold,
             nms_threshold, batch_size):

    # Compile model
    model = compile_model(model_path, anchors, grid_size, True, False,
                          len(labels), batch_size)
...
def main():
    ...
    with open(data_path, 'rb') as handle:
        train_data, valid_data, grid_size, labels = pickle.load(handle)
    ...
    if args.action != 'eval':
        ...
        model = train(args.model, train_data, valid_data, anchors, grid_size,
                      labels, args.obj_thresh, args.nms_thresh, epochs, tune,
                      freeze, args.batch_size)
        if args.savemodel:
        ...
        # Evaluate the model
        evaluate(args.model, valid_data, anchors, grid_size, labels,
                 args.obj_thresh, args.nms_thresh, args.batch_size)
