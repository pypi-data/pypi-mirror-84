        ...
        for funcstr, solchash in hashes.items():
            ...
            if funcstr in [x.signature() for x in funcs]:  # if function already appeared in ABI:
                ...
                def get_solidity_type_from_ast_param(p: Dict[str, Any]) -> SolidityType:
                    ...
                    is_struct = p_type.startswith("struct ")
                    ...
                    def collect_struct_member_types() -> List[SolidityType]:
                        ...
                        if is_struct:
                            ...
                            struct_def_node = {}  # type: Dict[str, Any]
                            for c in original_file_asts:
                                if struct_def_node_id in original_file_asts[c]:
                                    struct_def_node = original_file_asts[c][struct_def_node_id]
                                    assert ("nodeType" in struct_def_node and
                                            struct_def_node["nodeType"] == "StructDefinition")
                                    break
