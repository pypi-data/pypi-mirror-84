...
try:
    ...
    logging.getLogger(__name__).error("%s: %s. Required by graph/spatial module.", error.__class__.__name__, str(error))
    pass
...
class ConnectionContext(object):
    ...
    def sql(self, sql, **kwargs):
        ...
        if 'geo_cols' in kwargs:
            ...
            return DataFrame(self, sql, geo_cols=geo_cols)
        else:
...
#        return DataFrame(self.connection_context, new_select_statement)
    ...
    def save(self, where, table_type=None, force=False, save_source=True):
        """
