...
def discover_graph_workspaces(connection_context):
    ...
    per GWS:
        ...
        EDGE_SOURCE_COLUMN_NAME, EDGE_TARGET_COLUMN, EDGE_KEY_COLUMN_NAME, VERTEX_SCHEMA_NAME, VERTEX_TABLE_NAME,
...
def create_hana_graph_from_vertex_and_edge_frames(
    ...
    >>>  schema='SYSTEM', vertex_key_column=vertex_key_column, edge_source_col=edge_from_col, edge_target_col=edge_to_col,
...
class Graph(object):#pylint: disable=too-many-public-methods
        ...
        for dtype in self.vertices_hdf.dtypes():
            if dtype[0] == self.vertex_key_column:
                if dtype[1].upper() == 'NVARCHAR':
                    _cols.append('"{}" {}(32)'.format(dtype[0], dtype[1].upper()))
                else:
    ...
    def shortest_path(self, source, target, weight=None, direction=DEF_DIR):
        ...
        >>> nx_graph1 = nx.from_pandas_edgelist(path1.edges(), source=edge_source_col, target=edge_target_col)
