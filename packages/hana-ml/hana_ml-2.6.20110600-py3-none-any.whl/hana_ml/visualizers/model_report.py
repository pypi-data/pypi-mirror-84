...
class UnifiedClassificationReportBuilder(object):
    ...
    def build(self, metric_sampling=False):
        ...
        template = TemplateUtil.get_template('model_report.html')
        if metric_sampling is False:
            ...
            self.__report_sampling_html = template.render(
                title='Unified Classification',
                start_time=start_time,
                statistic=statistic_body_html,
                parameter=parameter_body_html,
                ...
                metrics=metrics_body_html,
                load_echarts_js=self.__all_js)
            self.__report_sampling_html = minify(self.__report_sampling_html, remove_all_empty_space=True, remove_comments=True)
    ...
    def generate_html_report(self, filename, metric_sampling=False):
        if metric_sampling:
            TemplateUtil.generate_html_file('{}_model_report.html'.format(filename), self.__report_sampling_html)
        else:
            TemplateUtil.generate_html_file('{}_model_report.html'.format(filename), self.__report_html)
