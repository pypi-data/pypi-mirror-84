...
class VectorARIMA(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    order : (p, q, d), tuple of int, optional
        ...
        Defaults to (-1, 0, -1). -1 indicates auto.
    ...
    seasonal_order : (P, D, Q, s), tuple of int, optional
        ...
        Defaults to (-1, 0, -1, 0). -1 indicates auto.
    ...
    lag_num : int, optional
        The lag number of explanatory variables. Valid only when model_type='VAR'.
    ...
    max_p : int, optional
        ...
        Defaults to 6 if model_type='VAR' or if model_type='VARMA' and search_method='eccm'.
        Defaults to 2 if model_type='VARMA' and search_method='grid_search'.

    max_q : int, optional

        The maximum value of vector MA order q.

        Defaults to 8 if model_type='VMA'.
        Defaults to 5 if model_type='VARMA' and search_method='eccm'.
        Defaults to 2 if model_type='VARMA' and search_method='grid_search'.
    ...
    max_seasonal_p : int, optional
        ...
        Defaults to 3 if model_type='VAR'.
        Defaults to 1 if model_type='VARMA' and search_method='grid_search'.
    ...
    max_lag_num : int, optional
        The maximum lag number of explanatory variables. Valid only when model_type='VAR'.
    ...
    finite_diff_accuracy : int, optional
        Polynomial order of finite difference. Approximate the gradient of objective function with finite difference. The valid range is from 1 to 4.
    ...
    calculate_hessian : bool, optional
        Specifies whether to calculate the Hessian matrix. VMA and VARMA will output standard error of parameter estimates only when calculate_hessian is True.
    ...
    irf_lags : int, optional
        The number of lags of the IRF to be calculated. Valid only when calculate_irf is True.
    ...
    Create an vector ARIMA instance:
    ...
    def fit(self, data, key=None, endog=None, exog=None):#pylint: disable=too-many-arguments,too-many-branches, too-many-statements
        ...
        endog : list of str, optional
            ...
            Defaults to the non-key columns of data excluding exogeneous columns if not provided.
