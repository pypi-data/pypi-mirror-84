...
class Detection:
  ...
  Example of searching for a file from a connector (ex. Box, Teams, S3, etc.)
  ```python
  ...
  Example of getting a list of reports
  ...
  def get_report(self, report_id, extended=False):
    ...
    Keyword Arguments:
        report_id {string} -- The ID of the report you wish to retrieve
        extended {boolean} -- Return a detailed report (default: False)
  ...
  def telemetry(self, **query_params):
    """Allows you to get one or more reports
    Keyword Arguments:
        query_params {**kwargs} -- One or more optional query parameters to include with the request

    Example: detection.telemetry(start_time=1603980060, end_time=1604066460)
    
    Returns:
        dict -- Returns the report details
    """
    return self.get(self.api_host, "/telemetry", query_params)

  def search_reports(self, connector_type, file_id, **query_params):
    """Allows you to find a report for a specific connector and file ID

    Keyword Arguments:
        connector_type {string} -- The type of connector to search for (ex. box, teams, etc.)
        file_id {string}        -- The ID of the file to search for as defined by the specific connector
        query_params {**kwargs} -- One or more optional query parameters to include with the request

    Example: detection.search_reports(connector_type="box", file_id="123456", extended=True)
    ...
    return self.get(self.api_host, "/reports/search", params)
