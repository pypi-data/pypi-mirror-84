...
class File(object):
    ...
    def create_directory(self):
        ...
        Sono ammesse solo chiamate POST
        :param apikey:     apikey che vuole effettuare l'azione
        :param path:       path della cartella da creare
        :param perms:      permessi della cartella da creare (default = None)
        :param overwrite:  indica se sovrascrivere la cartella nel caso esistesse (default = False)
        """
        if cherrypy.request.method != 'POST':
            # controllo se è una chiamata GET, in tal caso specifico che sono ammesse solo chiamate POST
            res = build_response(status=1, substatus=0, message="Only POST admitted")
