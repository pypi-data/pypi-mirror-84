...
def create_parser(parser, use_gooey=True):
    ...
    tomo_group.add_argument(
        ...
        type=int,
        default=-1,
        help="Search range in pixel. On default it will choose 25% of the box size.",
    )
    ...
    tomo_group.add_argument(
        ...
        type=int,
        default=5,
        help="The minimum number of boxes in one trace to be considered as valid particle."
    )
    ...
    if write_direct == False:
        ...
        if predict_3D:
            ...
            for result in picking_results:
                ...
                mean_box_size = np.mean([np.mean([(box.w+box.h)/2 for box in slice_boxes]) for slice_boxes in result["boxes"] if len(slice_boxes)>0])
                ...
                from cryolo.frontend import non_maxima_suppress_fast_3d
                before_fitlering = len(bounding_boxes_3d)
                bounding_boxes_3d = non_maxima_suppress_fast_3d(bounding_boxes_3d,
                                                     nms_threshold=0.3,
                                                     obj_threshold=obj_threshold)
                print("Removed by IOU:", before_fitlering-len(bounding_boxes_3d))
