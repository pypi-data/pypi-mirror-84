...
def read_eman1_boxfile(path):
    ...
    box_size_default=200
    if len_row == 2: #2D case without weight and height values
        boxes = [BoundBox(x=box[0], y=box[1], w=box_size_default, h=box_size_default) for box in boxreader]
    elif len_row == 4: #2D case with weight and height values
        boxes = [BoundBox(x=box[0], y=box[1], w=box[2], h=box[3]) for box in boxreader]
    elif len_row in [3,5]: #3D case without weight and height values
        w= box_size_default if len_row == 3 else boxreader[0][3]
        h = box_size_default if len_row == 3 else boxreader[0][4]
        for box in boxreader:
            b = BoundBox(x=box[0]-w/2, y=box[1]-h/2, w=w, h=h)
            b.z=box[2]
            boxes.append(b)
    return boxes
