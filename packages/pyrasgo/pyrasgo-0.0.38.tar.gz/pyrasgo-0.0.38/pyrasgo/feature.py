...
class FeatureList(Sequence):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Feature):
            return type(self)([feature.dict() for feature in self.data + [other]], api_key=self._api_key)
        if isinstance(other, type(self)):
            return type(self)([feature.dict() for feature in self.data + other.data], api_key=self._api_key)
...
class Feature(Connection):
    ...
    def build_stats(self):
        #call a lambda or api endpoint to kick off sql
        #generate json profile and store in s3
