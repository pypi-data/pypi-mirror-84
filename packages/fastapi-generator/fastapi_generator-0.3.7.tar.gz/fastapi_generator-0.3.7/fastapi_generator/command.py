...
@fastapi.command()
@click.option('--is_only_orm', is_flag=False)
...
def orm(db_name, db_host, db_user, db_pswd, is_only_orm):
    app = MainApp()
    app.create_app_with_orm(
        db_name=db_name, db_pswd=db_pswd, db_host=db_host, db_user=db_user, is_only_orm=is_only_orm
    )
