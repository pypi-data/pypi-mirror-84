...
class Voe(API):
    ...
    def get(self, event_no: int = None):
        ...
        log.debug(
            "Fetching CHIME/FRB VOEvent record for event number {}".format(
                event_no,
            )
        )
    ...
    def detection(self, payload: dict = None, debug: bool = True, ip_address: str = ""):
        ...
        class Detection:
            ...
            spectral_index = attribute(
                default=None,
                validator=instance_of(float),
            )
            spectral_index_error = attribute(
                default=None,
                validator=instance_of(float),
            )
            ra = attribute(
        ...
        # will break it
        detection = Detection(**payload.get("VOEvent"))
        # Make URL
        url = "/v1/voe/detection{}{}".format(
            "?debug=1" if debug else "",
            "?trial=1" if ip_address != "" else "",
        )
    ...
    def update(self, payload: dict = None, debug: bool = True, ip_address: str = ""):
        ...
        class Update:
            ...
            dispersion_smearing = attribute(
                validator=instance_of(float),
            )
            dispersion_smearing_error = attribute(
                validator=instance_of(float),
            )
            dm_gal_ne_2001_max = attribute(
                validator=instance_of(float),
            ...
            redshift_host = attribute(
                validator=instance_of(float),
            )
            redshift_host_error = attribute(
                validator=instance_of(float),
            )
            dm_index = attribute(
                validator=instance_of(float),
            )
            dm_index_error = attribute(
                validator=instance_of(float),
            )
            scattering_timescale = attribute(
                validator=instance_of(float),
            )
            scattering_timescale_error = attribute(
                validator=instance_of(float),
            )
            scattering_index = attribute(
                validator=instance_of(float),
            )
            scattering_index_error = attribute(
                validator=instance_of(float),
            )
            spectral_index = attribute(
                validator=instance_of(float),
            ...
            fluence_error = attribute(
                validator=instance_of(float),
            )
            linear_pol = attribute(
                validator=instance_of(float),
            )
            linear_pol_error = attribute(
                validator=instance_of(float),
            )
            circular_pol = attribute(
                validator=instance_of(float),
            )
            circular_pol_error = attribute(
                validator=instance_of(float),
            )
            # Rotation measure
            rm = attribute(
        ...
        # If it is malformed, this will raise a TypeError
        update = Update(**payload.get("VOEvent"))
        # Make URL
    ...
    def retraction(
        ...
        # If it is malformed, this will raise a TypeError
        retraction = Retraction(**payload.get("VOEvent"))
        # Make URL
