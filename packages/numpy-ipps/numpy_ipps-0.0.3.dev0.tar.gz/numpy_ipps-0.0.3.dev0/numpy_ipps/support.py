...
class CacheParams:
    ...
    def __init__(self):
        if not hasattr(_libipp.ipp_signal, "ippGetCacheParams"):
            _libipp.ffi.cdef(
                """
                typedef struct {
                    int type, level, size;
    ...
    def __call__(self):
        ipp_caches = _utils.new("IppCache**")
        _numpy_ipps.status = _libipp.ipp_signal.ippGetCacheParams(ipp_caches)
...
class CpuFeatures:
    ...
    def __init__(self):
        if not hasattr(_libipp.ipp_signal, "ippGetEnabledCpuFeatures"):
            _libipp.ffi.cdef("uint64_t ippGetEnabledCpuFeatures();")

    def __call__(self):
        enabled_features = _libipp.ipp_signal.ippGetEnabledCpuFeatures()
...
class CpuFreq:
    ...
    def __init__(self):
        if not hasattr(_libipp.ipp_signal, "ippGetCpuFreqMhz"):
            _libipp.ffi.cdef("int ippGetCpuFreqMhz(int*);")

    def __call__(self):
        ipp_freq = _utils.new("int*")
        _numpy_ipps.status = _libipp.ipp_signal.ippGetCpuFreqMhz(ipp_freq)
...
class LibVersion:
    ...
    def __init__(self):
        if not hasattr(_libipp.ipp_signal, "ippGetLibVersion"):
            _libipp.ffi.cdef(
                """
                typedef struct {
    ...
    def __call__(self):
        ipp_lib_version = _libipp.ipp_signal.ippGetLibVersion()
