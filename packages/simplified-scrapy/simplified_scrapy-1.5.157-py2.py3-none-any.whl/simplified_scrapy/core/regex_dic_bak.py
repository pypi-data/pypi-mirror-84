...
class RegexDict(Dict):
    ...
    def setContent(self, text):
        ...
        _updatePosition(self._rootNode,
                        start + 1,
                        end,
                        len(text) - oldLen,
                        mod=True)
    ...
    def _setAttr(self, key, value):
        ...
        if value == None:
            ...
            self[key] = value
        _updatePosition(self._rootNode, self._start, start2,
                        html.find('>') - start2)
    ...
    def insertBefore(self, html):
        if isinstance(html, Dict):
            html = html.selfHtml
        h = self._rootNode.html
        h = h[0:self._start] + html + h[self._start:]
    ...
    def insertAfter(self, html):
        if isinstance(html, Dict):
            html = html.selfHtml
        h = self._rootNode.html
        h = h[0:self._end] + html + h[self._end:]
        self._rootNode['html'] = h
        _updatePosition(self._rootNode, self._end, self._end, len(html))

    def remove(self):
        removeKey = self._start
        h = self._rootNode.html
        s = h.rfind('\n', 0, self._start)
        e = h.find('\n', self._end)
        if s >= 0 and e >= 0:
        ...
        self._rootNode['html'] = h
        del self._rootNode._elements[removeKey]
        _updatePosition(self._rootNode,
                        self._start,
                        self._end,
    ...
    def replaceSelf(self, html):
        ...
        self._rootNode['html'] = h
        del self._rootNode._elements[self._start]
        _updatePosition(self._rootNode,
                        self._start,
                        self._end,
                        len(html) - self._end + self._start,
                        mod=True)
        self.clear()
...
def _updatePosition(root, start, end, size, mod=False):
    changes = {}
    dels = []
    _end = start + size  # new end
    eles = root._elements
    for k, e in eles.items():
        if e._end < start:
            print('1')
            pass
        elif e._start < start:
            if e._end < end:
                ...
                e._end = e._end + size
                s = root.html.find('>', e._start)
                d = root.html.rfind('<', e._start, e._end)
                e['html'] = root.html[s:d]
        elif e._start >= end:
            dels.append(e._start)
            e._start = e._start + size
            e._end = e._end + size

            changes[e._start] = e
        elif e._start > start and e._end < end:
            dels.append(e._start)
        else:
            print('3')
            pass
