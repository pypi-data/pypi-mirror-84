...
class RabbitMQConfigFileOptions(Options):
    """
...
class RabbitMQConfigFile(ConfigFile):
    """
    RabbitMQ main configuration file in SysCtl format (3.7.0 or higher).
    """
    options: RabbitMQConfigFileOptions

    def __init__(self, options: Optional[RabbitMQConfigFileOptions] = None):
        if options is None:
            options = RabbitMQConfigFileOptions()
