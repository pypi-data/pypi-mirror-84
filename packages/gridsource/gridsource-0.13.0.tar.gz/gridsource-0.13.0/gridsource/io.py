...
class IOMixin:
    ...
    def read(self, fpath, engine=DEFAULT_EXCEL_ENGINE):
        """write to the given file, guessing format from the extension"""
        fmts = {
            ".xlsx": (self.read_excel, {"fpath": fpath, "engine": engine}),
            ".cfg": (self.read_configobj, {"fpath": fpath}),
            ".ini": (self.read_configobj, {"fpath": fpath}),
        }
    ...
    def read_excel(self, fpath, engine=DEFAULT_EXCEL_ENGINE):
        """read excel-like file and store all the tabs
        as pandas DataFrame values"""
        self.reset()
        self._data = pd.read_excel(fpath, sheet_name=None, engine=engine)
    ...
    def to_configobj(self, fpath):
        ...
        config = ConfigObj(indent_type="    ", unrepr=True, write_empty_values=True)
    ...
    def read_configobj(self, fpath, row_prefix="sniff"):
        if not IS_CONFIGOBJ:
            raise ValueError(
                "Cannot export to configobj. Please install configobj before"
            )
        self.reset()
        config = ConfigObj(
            fpath, indent_type="    ", unrepr=True, write_empty_values=True
        )
        # transform sections to tabs and data to columns...
        if row_prefix == "sniff":
            row_prefix = _sniff_row_prefix(config[config.sections[0]].sections[0])
            # # sniff row_prefix
            # first_item = config[config.sections[0]].sections[0]  # eg.: "_row_0"
            # row_prefix = re.match(r"^(.*)\d+$", first_item).group(1)
        else:
            row_prefix = ROW_PREFIX
        for tabname, data in config.items():
            self._data[tabname] = _cfg_section_to_df(tabname, data, len(row_prefix))
