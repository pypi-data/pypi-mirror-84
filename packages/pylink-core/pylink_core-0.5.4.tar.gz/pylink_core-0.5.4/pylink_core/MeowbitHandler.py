...
class MeowbitHandler(HardwareHandler):
  ...
  def sync_files(self):
    ...
    try:
      ...
      boardfile = set(boardfile).difference(skipList)
      print(boardfile)
      ...
      self.sendResp(self.pid, None, {"error": str(e), "code": KERR.SYNC_FILE_FAIL})
    finally:
      self.sendReq("upload-status", {})
      self.comm.setPybMutex(False)
  ...
  def sync_files_pyb(self, params):
    try:
      ...
      try:
        ...
        print(boardfile)  # 拿到掌机内的json文件
        fp.rm('filedict.json')
      except RuntimeError as err:
        
        for pybfile in realRootFile:
          # pybfile = pybfile[1:].split(' - ')[0]
          if pybfile not in skipList:
            fp.rm(pybfile)
        print(str(err))
        boardfile = {}

      # primitiveRealRootFile = fp.ls()
