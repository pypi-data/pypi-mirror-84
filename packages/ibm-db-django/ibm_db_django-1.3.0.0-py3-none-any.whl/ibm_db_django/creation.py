...
class DatabaseCreation ( BaseDatabaseCreation ):
    ...
    def create_test_db( self, verbosity = 0, autoclobber = False , keepdb=False,serialize=False):
        ...
        if not _IS_JYTHON:
            ...
            if not autoclobber:
                confirm = eval(input( "Wants to create %s as test database. Type yes to create it else type no" % ( kwargs.get( 'database' ) ) ))
            if autoclobber or confirm == 'yes':
                try:
                    ...
                    if ( message.find( 'Not supported:' ) != -1 ):
                        if not autoclobber:
                            confirm = eval(input( "Not able to create test database, %s. Type yes to use %s as test database, or no to exit" % ( message.split( ":" )[1], old_database ) ))
                        else:
                            confirm = eval(input( "Not able to create test database, %s. Type yes to use %s as test database, or no to exit" % ( message.split( ":" )[1], old_database ) ))
                        if autoclobber or confirm == 'yes':
                ...
                confirm = eval(input("Wants to use %s as test database, Type yes to use it as test database or no to exit" % ( old_database ) ))
                if confirm == 'yes':
                    kwargs['database'] = old_database
                    self.__clean_up( self.connection.cursor() )
                    self.connection._commit()
