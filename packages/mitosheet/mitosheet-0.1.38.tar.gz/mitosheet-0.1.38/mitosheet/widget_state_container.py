...
from mitosheet.save_utils import write_analysis, read_analysis
...
class WidgetStateContainer():
    ...
    def __init__(self, dfs: pd.DataFrame):
        ...
        self.steps = [{
            ...
            'column_filters': [{key: {"condition": 'none', 'value': '', 'type': get_column_filter_type(df[key])} for key in df.keys()} for df in dfs],
            'dfs': dfs
        }]
    ...
    def handle_edit_event(self, edit_event):
        ...
        elif edit_event['type'] == 'filter':
            self.filter(
                edit_event['sheet_index'],
                edit_event['column'],
                edit_event['condition'],
                edit_event['value']
            )
        else:
    ...
    def _reset_filter(self, sheet_index, column):
        """
        ...
        for step_id, step in enumerate(self.steps):
            if step['step_type'] == 'filter' and step_id != self.curr_step_id:
                # If this is the step that added a filter to the column, mark it as the one to delete
                if step['sheet_index'] == sheet_index and step['column'] == column:
        ...
        self.steps.pop(deleted_step_id)
        ...
        # Rerun the analysis
        saved_analysis = read_analysis(self.analysis_name)
        self._rerun_analysis(saved_analysis['steps'])
    ...
    def add_column(self, sheet_index: int, column_header: str):
        ...
        self.curr_step['column_filters'][sheet_index][column_header] = {"condition": 'none', 'value': '', 'type': 'number'}
    ...
    def set_column_formula(
        ...
        # Finially, update the type of the column
        self.curr_step['column_filters'][sheet_index][column_header]['type'] = get_column_filter_type(self.curr_step['dfs'][sheet_index][column_header])
    ...
    def merge_sheets(
        ...
        self.curr_step['column_filters'].append({column_header: {"condition": 'none', 'value': '', 'type': get_column_filter_type(new_df[column_header])} for column_header in column_headers})


        # after done merging, we create and checkout a new formula step!
        self._create_and_checkout_new_step(self.curr_step_id + 1, 'formula')

    def filter(self, sheet_index, column, condition, value):
        """
        Filters an existing sheet with the given condition at the given value.
        """
        # if the sheet doesn't exist, throw an error
        if not self.does_sheet_index_exist(sheet_index):
            raise make_no_sheet_error(sheet_index)

        # We check that the filtered column exists 
        missing_column = set([column]).difference(self.curr_step['column_metatype'][sheet_index].keys())
        ...
        if condition == 'none':
            ...
            self._reset_filter(sheet_index, column)
            return

        # If no errors we create a new step for this filter
        self._create_and_checkout_new_step(self.curr_step_id + 1, 'filter')

        # Then we update the dataframe, first by executing on a fake dataframe
        try:

            # make a copy of our data frame to test operate on 
            df_copy = deepcopy(self.curr_step['dfs'][sheet_index])
        ...
        # update the step to save the variables needed to reconstruct the merge
        self.curr_step['sheet_index'] = sheet_index
        self.curr_step['column'] = column
        self.curr_step['condition'] = condition
        self.curr_step['value'] = value

        # if there is a filter already applied to the column, remove it
        self._reset_filter(sheet_index, column)

        # if there was no error in execution on the copy, execute on real dataframes
        self.dfs[sheet_index] = self._execute_filter(
            self.dfs[sheet_index], 
            column,
            condition,
            value 
        )
        ...
        # Now, we build the filter code
        if condition == 'none':
            filter_code= ['']
        elif condition == 'number_exactly':
            filter_code = [f'{df_name}[{df_name}[\'{column}\'] == {value}]']
            ...
            filter_code = [f'{df_name}.dropna(subset=[\'{column}\'])']
        elif condition == 'not_empty':
            filter_code = [f'{df_name}[{df_name}.{column}.notnull()]']
        elif condition == 'contains':
            # TODO: fix this only works on strings, not number
            # error: Can only use .str accessor with string values!
            filter_code = [f'{df_name}[{df_name}[\'{column}\'].str.contains(\'{value}\', na=False)]']
        elif condition == 'string_exactly':
            filter_code = [f'{df_name}[{df_name}[\'{column}\'] == \'{value}\']']
        else: 
            raise Exception("Condition passed to filter is unsupported:", condition)
    
        # And then save it
        self.curr_step['filter_code'] = filter_code
        ...
        self.curr_step['column_filters'][sheet_index][column]['condition'] = condition
        self.curr_step['column_filters'][sheet_index][column]['value'] = value
    ...
    def _execute_filter(
        ...
        # perform the correct filter based on the condition
        if condition == 'none':
            pass
        elif condition == 'number_exactly':
            ...
            raise Exception("condition passed to filter is unsupported")
