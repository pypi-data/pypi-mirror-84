...
def test_delete_filter():
    ...
    wsc = WidgetStateContainer([df])
    wsc.add_column(0, 'B')
    wsc.set_column_formula(0, 'B', '=0', '=A')

    # add a filter
    wsc.filter(0, 'B', 'greater', 2)

    # add and edit a column after the filter
    wsc.add_column(0, 'C')
    wsc.set_column_formula(0, 'C', '=0', '=9')

    # reset the filter
    wsc._reset_filter(0, 'B')

    assert len(wsc.steps) == 1
...
def test_delete_filter_no_effect():
    ...
    # reset the filter
    wsc._reset_filter(0, 'C')

    # reseting the filter of a column that has no filter, should have no effect
    assert len(wsc.steps) == 3
    assert wsc.steps[1]['column_filters'][0]['B']['condition'] == 'greater'
    assert wsc.steps[1]['column_filters'][0]['B']['value'] == 2
