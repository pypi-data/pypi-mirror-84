...
class ModelTestCase(unittest.TestCase):
    ...
    def test_model_build_segments(self):
        model = self._client.models(project='learn_kylin', name='kylin_sales_cube')[0]
        start = datetime(2020, 5, 1, 8)
        end = datetime(2020, 5, 2, 8)
        model.segments(start_time=start, end_time=end)
