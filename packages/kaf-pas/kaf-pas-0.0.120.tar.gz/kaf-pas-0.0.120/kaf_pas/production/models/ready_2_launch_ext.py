                ...
                for item_ref in descendant_request:
                    ...
                    try:
                        ...
                        if inner_descendant_count != inner_lines_count:
                            notes.push(blinkString(text=f'Количетво дочерних узлов в дереве состава изделия ({inner_descendant_count}), не равно количеству строк детализации ({inner_lines_count}).', blink=False, color=black, bold=False))
                        ...
                        if section and section != 'Документация':
                            if item_ref.props.used.is_set == False or items_not_used.exists(lambda x: x == item_ref.parent.id):
                                notes.push(blinkString(text='Позиция не используется.', blink=False, color=black, bold=False))
                                if not item_full_name:
                                    item_full_name_obj = items_refs_stack.get_full_path_obj
                                    item_full_name = items_refs_stack.get_full_path
                                ...
                                cnt_not1 += 1
                                notes.push(blinkString(text='Не указано количество.', blink=False, color=red, bold=False))
                                if not item_full_name:
                                    item_full_name_obj = items_refs_stack.get_full_path_obj
                                    item_full_name = items_refs_stack.get_full_path
                        else:
                            notes.push(blinkString(text='Найдена документация, требует проверки правильности принадлежности.', blink=False, color=black, bold=False))
                            if not item_full_name:
                                item_full_name_obj = items_refs_stack.get_full_path_obj
                                item_full_name = items_refs_stack.get_full_path
                        ...
                        if demand is not None:
                            cnt_not += 1
                            notes.push(blinkString(text='Не входит в детализацию.', blink=False, color=red, bold=False))
                            if not item_full_name:
                                item_full_name_obj = items_refs_stack.get_full_path_obj
                                item_full_name = items_refs_stack.get_full_path
                    ...
                    if item_ref.props.used.is_set == True and not items_not_used.exists(lambda x: x == item_ref.parent.id):
                        if section and section != 'Документация':
                            if operations_cnt == 0:
                                if IsBitOn(props, 5):
                                    cnt_not += 1
                                    notes.push(blinkString(text='Не указаны операции.', blink=False, color=red, bold=False))
                                    item_full_name_obj = items_refs_stack.get_full_path_obj
                                    item_full_name = items_refs_stack.get_full_path
                            else:
                                for operations_item in Operations_item.objects.filter(item=item_ref.child):
                                    if IsBitOn(props, 0) and not operations_item.qty:
                                        cnt_not1 = 1
                                        notes.push(blinkString(text=f'Операция: {operations_item.operation.full_name} не указана длительность.', blink=False, color=red, bold=False))
                                        if not item_full_name:
                                            item_full_name_obj = items_refs_stack.get_full_path_obj
                                            item_full_name = items_refs_stack.get_full_path
                                    ...
                                    if IsBitOn(props, 1):
                                        if not operations_item.num:
                                            cnt_not1 = 1
                                            notes.push(blinkString(text=f'Операция: {operations_item.operation.full_name} не указан № п/п.', blink=False, color=red, bold=False))
                                            if not item_full_name:
                                                item_full_name_obj = items_refs_stack.get_full_path_obj
                                                item_full_name = items_refs_stack.get_full_path
                                    ...
                                    if IsBitOn(props, 2):
                                        ...
                                        if operation_material_cnt == 0:
                                            cnt_not1 = 1
                                            notes.push(blinkString(text=f'Операция: {operations_item.operation.full_name} не указаны материалы или стандартные изделия.', blink=False, color=red, bold=False))
                                            if not item_full_name:
                                                item_full_name_obj = items_refs_stack.get_full_path_obj
                                                item_full_name = items_refs_stack.get_full_path
                                    ...
                                    if IsBitOn(props, 3):
                                        ...
                                        if operation_resources_query.count() == 0:
                                            cnt_not1 = 1
                                            notes.push(blinkString(text=f'Операция: {operations_item.operation.full_name} не указан ресурс либо место выполнения.', blink=False, color=red, bold=False))
                                            if not item_full_name:
                                                item_full_name_obj = items_refs_stack.get_full_path_obj
                                                item_full_name = items_refs_stack.get_full_path
                                        else:
                                                ...
                                                if operation_resources.location_fin is None and operation_resources.resource_fin is None:
                                                    ...
                                                    try:
                                                        ...
                                                        if operation_def_resources.location_fin is None and operation_def_resources.resource_fin is None:
                                                            cnt_not1 = 1
                                                            notes.push(blinkString(text=f'Операция: {operations_item.operation.full_name} не указан конечный ресурс либо место выполнения.', blink=False, color=red, bold=False))
                                                            if not item_full_name:
                                                                item_full_name_obj = items_refs_stack.get_full_path_obj
                                                                item_full_name = items_refs_stack.get_full_path
                                                        else:
                                    ...
                                    if IsBitOn(props, 4) and not operations_item.ed_izm:
                                        cnt_not1 = 1
                                        notes.push(blinkString(text=f'Операция: {operations_item.operation.full_name} не указана единица измерения.', blink=False, color=red, bold=False))
                                        if not item_full_name:
                                            item_full_name_obj = items_refs_stack.get_full_path_obj
                                            item_full_name = items_refs_stack.get_full_path
                    else:
