...
class Constraints:
    ...
    # New methods -------------------------------------------------------------
    def add_constraint(self, a: np.ndarray, b: np.ndarray) -> None:
    ...
    def set_value(self, coord: str, c: str, value: Real = 0.0) -> None:
        a = self.blank()
        a[self.index(coord, c)] = 1.0
        self.add_constraint(a, value)

    def set_delta(self, coord: str, *args: str, delta: Union[Real, Iterable[Real]] = 0.0) -> None:
        if len(args) <= 1:
            raise ValueError
        deltas: Iterable[Real]
    ...
    def set_deltas(self,
                   *args: str,
                   delta: Union[Tuple[Real, Real],
                                Sequence[Tuple[Real, Real]]] = (0.0, 0.0)) -> None:
    ...
    def set_x(self, c: str, x: Real) -> None:
        self.set_value('x', c, x)

    def set_y(self, c: str, y: Real) -> None:
        self.set_value('y', c, y)

    def set_delta_x(self, *args: str, delta_x: Union[Reversible, Real] = 0.0) -> None:
        """
        Stack each of the args horizontally, spaced by delta_x, from left to
        right.
        """
        if isinstance(delta_x, Reversible):
            delta_x = reversed(delta_x)
        self.set_delta('x', *reversed(args), delta=delta_x)

    def set_delta_y(self, *args: str, delta_y: Real = 0.0) -> None:
        """
    ...
    def set_location(self, c: str, location: Tuple[Real, Real]) -> None:
    ...
    def set_between(self, coord: str, *items: str, ratios: Optional[Sequence[Real]] = None) -> None:
        """
    ...
    def set_x_between(self, *items: str, ratios: Optional[Sequence[Real]] = None) -> None:
        self.set_between('x', *items, ratios=ratios)

    def set_y_between(self, *items: str, ratios: Optional[Sequence[Real]] = None) -> None:
    ...
    def set_slope(self, slope: Real, *args: str) -> None:
        if len(args) < 2:
            raise ValueError
