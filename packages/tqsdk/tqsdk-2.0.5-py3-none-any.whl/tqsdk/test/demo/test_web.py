...
def run_tianqin_code(port, queue, ins_port):
    try:
        ...
        api = TqApi(auth="tianqin,tianqin", web_gui="127.0.0.1:" + port, _md_url="wss://nfmd.shinnytech.com/t/nfmd/front/mobile")
        queue.put("webready")
        klines = api.get_kline_serial("SHFE.au2010", 24 * 60 * 60)
...
def run_for_driver(driver, test):
    ...
    up_line = chart_main_candle.find_element_by_css_selector("path.candle.line.up")
    down_line = chart_main_candle.find_element_by_css_selector("path.candle.line.down")
