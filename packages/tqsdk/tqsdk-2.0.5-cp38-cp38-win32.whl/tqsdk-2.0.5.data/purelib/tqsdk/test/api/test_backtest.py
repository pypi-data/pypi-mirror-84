...
class TestTdBacktest(unittest.TestCase):
    ...
    def test_backtest(self):
        def backtest():
            ...
            times = []
            for i in range(20):
                mock = MockServer(md_url_character="nfmd")
                mock.run(log_path)
                md_url = f"ws://127.0.0.1:{mock.md_port}/"
                td_url = f"ws://127.0.0.1:{mock.td_port}/"
                utils.RD = random.Random(4)
                try:
                    backtest = TqBacktest(start_dt=datetime.datetime(2020, 6, 1), end_dt=datetime.datetime(2020, 7, 1))
                    api = TqApi(backtest=backtest, auth="tianqin,tianqin", _md_url=md_url, _td_url=td_url)
        ...
        self.assertLess(peakmemory, 3 * 1024)  # 内存使用不超过 1.5 GB
