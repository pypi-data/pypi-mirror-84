...
def run_single(
    *,
    ...
    elogger.debug(f"The containers to monitor: {tomonitor}")
    ...
    #
    # if do_pull:
    #     from duckietown_challenges_cli.cli_pull import docker_pull_retry
...
def get_services_id(wd, project, services: List[str]) -> Dict[str, str]:
    cmd = ["ps", "-q"]
    cmd.extend(services)
...
def write_logs_worker(service_name: str, container_id: str, color: str, l: int):
    # elogger.info('writing logs for %s ' % container_id)
    scol = termcolor.colored(service_name.ljust(l), color)
    ...
    elogger.debug(f"write_logs_worker: container {service_name} terminated with {exit_code}")
...
def stream_logs(service_name: str, container_id: str, color: str, l: int):
    from multiprocessing import Process

    p = Process(target=write_logs_worker, args=(service_name, container_id, color, l))
    p.start()
...
def run_one(
    ...
    try:
        ...
        run_docker(wd, project, cmd)
        elogger.debug(f"Creating containers {services}")
        cmd = [
            "up",
            "--no-start",
            "--renew-anon-volumes",
        ...
        for service_name, container_id in service2id.items():
            ...
            # elogger.debug(f"container ports: {ports}")
        ...
        elogger.debug(f"Starting containers {services}")
        cmd = [
            "up",
            "-d",
        ]
        ...
        for i, (service_name, service_id) in enumerate(service2id.items()):
            ...
            stream_logs(service_name, service_id, color=color, l=l)
        exited2 = {}
        while True:
            if heartbeat:
                heartbeat()
            ...
            for service_name, container_id in service2id.items():
                ...
                running_details[service_name] = f"{memory_stats}; cpu {cpu_stats}"
                # elogger.debug(container_id=container_id, stats=stats)
            ...
            delta = time.time() - t0
            msg = f"Running for {int(delta)} s (timeout: {int(timeout_sec)} s). Services running: {running}  "
            elogger.debug(msg, running_details=running_details, exited=exited2)

            if monitor in exited2:
                elogger.info(f"The container to monitor {monitor!r} has exited; bye bye.", exited=exited2)
                break
...
def get_config(
    ...
    write_ustring_to_utf8_file("runner", os.path.join(fd, "runner"))
    write_ustring_to_utf8_file("runner", os.path.join(wd, "runner"))
    ...
    if debug_volumes or ("DT_MOUNT" in os.environ):
        elogger.debug("Mounting local sources.")
...
def write_logs(wd, project, services: List[str]):
    client = check_docker_environment()
