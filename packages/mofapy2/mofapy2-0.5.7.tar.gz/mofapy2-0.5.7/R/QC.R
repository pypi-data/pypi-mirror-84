...
.quality_control <- function(object, verbose = FALSE) {
  ...
  if (any(stringi::stri_enc_mark(unlist(samples_names(object))))) {
    stop("non-ascii characters detected in your samples names, please rename")
  }
  ...
  if (any(stringi::stri_enc_mark(unlist(features_names(object))))) {
    stop("non-ascii characters detected in your features names, please rename")
  }
