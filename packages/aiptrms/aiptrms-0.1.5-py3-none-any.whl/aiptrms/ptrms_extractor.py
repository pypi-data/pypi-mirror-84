...
class Ptrms:
    ...
    def extract_windows(self):
        ref_window_stats = (
            self.frame.loc[int(self.ref_start) - 1: int(self.ref_end)+1]
                .drop(columns=["Cycle"], errors="ignore")
                .describe()
        )
        sample_window_stats = (
            self.frame.loc[int(self.sample_start) - 1: int(self.sample_end)+1]
                .drop(columns=["Cycle"], errors="ignore")
                .describe()
        )

        self.dif = sample_window_stats.transpose().sub(ref_window_stats.transpose())
        self.dif = self.dif.drop(columns=['std'], errors='ignore')
        self.dif['Sample STD'] = sample_window_stats['std']
        # self.dif['Reference STD'] = ref_window_stats['std']
        # self.dif['Sample Max'] = sample_window_stats['max']
        # self.dif['Reference Max'] = ref_window_stats['max']
