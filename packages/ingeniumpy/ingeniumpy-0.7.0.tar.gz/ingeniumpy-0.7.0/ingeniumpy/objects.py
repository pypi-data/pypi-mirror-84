...
class IngComponentType(Enum):
    ...
    def is_air_sensor(self): return self in [self.COD_AIR_SENSOR]
...
class IngAirSensor(IngObject):
    ...
    async def update_state(self, p: Package) -> bool:
        ...
        return self.bit_values[p.data1] != old_value

    def get_available(self, mode: int):
        v1 = self.bit_values[2 * mode]
        v2 = self.bit_values[2 * mode + 1]
        return not ((v1 == 255 and v2 == 255) or (v1 == 0 and v2 == 0))

    def get_value(self, mode):
        return self.values[mode]

    def get_threshold_available(self, mode: int):
        v1 = self.bit_values[10 + 2 * mode]
        v2 = self.bit_values[10 + 2 * mode + 1]
    ...
    async def set_threshold(self, mode, threshold):
        self.bit_values[10 + 2 * mode] = threshold >> 8
        self.bit_values[10 + 2 * mode + 1] = threshold & 0xFF
        self.thresh_values[mode] = threshold

        p = Package(0xFFFF, self.address, 4, 10 + 2 * mode, self.bit_values[10 + 2 * mode])
        await self.api.send(p)

        p = Package(0xFFFF, self.address, 4, 10 + 2 * mode + 1, self.bit_values[10 + 2 * mode + 1])
        await self.api.send(p)
