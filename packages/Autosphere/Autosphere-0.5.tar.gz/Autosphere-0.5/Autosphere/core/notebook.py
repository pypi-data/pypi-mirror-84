...
def print_precheck(f):
    ...
    def wrapper(*args, **kwargs):
        if not IPYTHON_AVAILABLE:
            return None
        output_level = os.getenv("AutoSphere_NOTEBOOK_OUTPUT_LEVEL", "1")
        if output_level == "0":
            return None
...
def notebook_table(table, count: int = 20):
    """Display Autosphere.Table as IPython Markdown object in the notebook

    :param table: `Autosphere.Table` object to print
    """
    # pylint: disable=C0415,E0611
    from Autosphere.Tables import Tables  # noqa
...
def _get_table_output(table):
    # pylint: disable=C0415,E0611
    from Autosphere.Tables import Tables, Table  # noqa

    output = ""
    try:
        if isinstance(table, Table):
            output = "<table class='AutoSpherefw-notebook-table'>"
...
def _get_markdown(**kwargs):
    ...
    for key, val in kwargs.items():
        if key == "text":
            output += f"<span class='AutoSpherefw-notebook-text'>{val}</span><br>"
        if key == "image":
            output += f"<img class='AutoSpherefw-notebook-image' src='{val}'><br>"
        if key == "link":
            link_text = (val[:75] + "..") if len(val) > 75 else val
            output += f"<a class='AutoSpherefw-notebook-link' href='{val}'>{link_text}</a><br>"
        if key == "table":
