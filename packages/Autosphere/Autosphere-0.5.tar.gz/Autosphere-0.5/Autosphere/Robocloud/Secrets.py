...
class FileSecrets(BaseSecretManager):
    ...
    environment variable ``AutoSphere_SECRET_FILE``, or as
    ...
    def __init__(self, secret_file="secrets.json"):
        ...
        self.path = required_env("AutoSphere_SECRET_FILE", secret_file)
...
class Secrets:
    ...
    variable ``AutoSphere_SECRET_MANAGER``, or the keyword argument ``default_adapter``.
    ...
    def __init__(self, *args, **kwargs):
        ...
        adapter = required_env("AutoSphere_SECRET_MANAGER", default)

        self._adapter_factory = self._create_factory(adapter, args, kwargs)
        self._adapter = None

        try:
            BuiltIn().import_library("Autosphere.RobotLogListener")
