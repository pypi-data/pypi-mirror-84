...
def ready_to_install():
    if os.path.isfile("C:\ProgramData\AGCO Corporation\AGCO Update\config.ini"):
        try:
            parser = configparser.ConfigParser()
            parser.read(r'C:\ProgramData\AGCO Corporation\AGCO Update\config.ini')
...
# I don't think this is necessary as it will automatically do this after it is done installing
...
def set_config_ini_to_install():
    logging.info('Setting IsExplicitInstallRunning to True in the AUC config.ini')
    with fileinput.FileInput(r'C:\ProgramData\AGCO Corporation\AGCO Update\config.ini', inplace=True,
                             backup='.bak') as file:
        for line in file:
...
def clear_auc_download_schedule():
    logging.info('Removing the toggle to pause AUC downloads')
    with fileinput.FileInput(r'C:\ProgramData\AGCO Corporation\AGCO Update\config.ini', inplace=True,
                             backup='.bak') as file:
        for line in file:
...
def bypass_download_scheduler():
    ...
    try:
        ...
        click.secho(
            f'AGCO Update was not found in the registry. Please confirm that you have AGCO update client installed. {e}',
            fg='red')
...
def get_state():
    ...
    elif install_ready and edt_installed and number_of_packages != 0:
...
def main(env, auc_env_change, updategroup, m_id, m_token) -> None:
    ...
    apply_certs()
    if not os.path.isdir('C:\ProgramData\AGCO Corporation\AGCO Update'):
        download_auc_client()
        install_auc()
    cid = get_client_id()
