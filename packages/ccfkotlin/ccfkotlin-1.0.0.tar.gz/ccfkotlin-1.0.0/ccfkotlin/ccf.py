...
class CodeConventionsFixer:
    ...
    def format_tokens(self, path):
        ...
        with open(path, 'r', encoding="utf-8") as file:
            ...
            tokens = list(filter(lambda x: x.token_type != TokenType.WHITESPACE,# and x.token_type != TokenType.NEW_LINE,
                                 orig_tokens))
            for i in range(0, len(tokens)):
                ...
                if token.value == "{":
                    ...
                    for name in refactor_indents.keys():
                        ...
                        if level is not None and level < indent_level:
    ...
    def write_log(self, file_path, line_number, error_type, error_val, correct_val):
        ...
        if self.ccf_type == CCFType.FIX:
            logging.error(file_path + ": " + str(line_number) + "-" + error_type + ": " + error_val + " renamed to " + correct_val)
        else:
            logging.warning(file_path + ": " + str(line_number) + "-" + error_type + ": " + error_val + " should be " + correct_val)
    ...
    def is_upper_snake_case(self, value: str):
        result = re.match(r'[A-Z]+(_[A-Z]+)*', value)
