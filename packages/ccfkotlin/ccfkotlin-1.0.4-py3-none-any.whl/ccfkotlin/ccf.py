...
class CodeConventionsFixer:
            ...
            for i in range(0, len(tokens)):
                ...
                if token.value == "{":
                    ...
                    if token.value == "val":
                        ...
                        if tokens[i + 1].token_type == TokenType.IDENTIFIER and not \
                                self.is_pascal_case(tokens[i + 1].value):
                            refactor_names[tokens[i + 1].value] = self.to_pascal_case(tokens[i + 1].value)
                            refactor_indents[tokens[i + 1].value] = indent_level
                            self.refactor_prev(tokens[last_bracket_index + 1: i], tokens[i + 1].value,
                                               self.to_pascal_case(tokens[i + 1].value))
    ...
    def is_imported(self, imp, package, class_name):
        ...
        if i == len(package) and i < len(imp) and class_name != "" and imp[i: len(imp)] == "." + class_name:
            return True
        elif i < len(imp) and ((imp[i - 1] == "." and imp[i] == "*") or
                               (i + 1 < len(imp) and imp[i] == "." and imp[i + 1] == "*")):
            return True
        return False
    ...
    def format_project(self, path):
        ...
        if self.ccf_type == CCFType.FIX:
            while True:
                ...
                for d in tree:
                    cur_dir_name = d[0]
                    dir_path = cur_dir_name.split("/")
                    if not self.is_camel_case(dir_path[len(dir_path) - 1]):
                        ...
                        os.rename(cur_dir_name, "/".join(dir_path))
                        continue
                break
            ...
            for file in cur_dir_files:
                if file.endswith(".kt"):
                    if not self.is_pascal_case(file[0: len(file) - 3]):
                        if self.ccf_type == CCFType.FIX:
                            os.rename(cur_dir_name + '/' + file, cur_dir_name + '/'
                                      + self.to_pascal_case(file[0: len(file) - 3]) + ".kt")
                        self.write_log(cur_dir_name + '/' + file, 0, "FILE_NAMING_ERROR",
                                       file, self.to_pascal_case(file[0: len(file) - 3]) + ".kt")
                        if self.ccf_type == CCFType.FIX:
                            files.append(cur_dir_name + '/' + self.to_pascal_case(file[0: len(file) - 3]) + ".kt")
                        else:
