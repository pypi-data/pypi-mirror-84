...
def subprocess_cmd(command):
    ...
    """
    process = subprocess.Popen(command, stdout=None, shell=True)
...
def inside_box(span, mesh, include_zero_size=False):
    ...
    # Include zero span as a single pixel if requested
    span_tmp = np.copy(span)
    if include_zero_size==True:
        for dim in range(3):
            if span_tmp[dim, 1] - span_tmp[dim, 0] < fp_eps:
                # Take the pixel width at the correct location, in case of 
                # non-uniform mesh. Just take the first element of the mesh 
                # that is larger than the requested min value. 
                ind = np.nonzero(span_tmp[dim, 0] < mesh[dim])[0][0]
                span_tmp[dim, 0] = mesh[dim][ind] - fp_eps
                span_tmp[dim, 1] = mesh[dim][ind] + fp_eps
...
def nk2epssig(n, k, freq):
    ...
    -------
    float
        eps_real : Real part of the relative permittivity.
        sigma : Conductivity in S/micron.
    """
    eps_real = n**2 - k**2
    eps_imag = 2*n*k
