...
#            row[0].page_number,
    ...
    if as_text:
        return get_text_from_table(table)
...
def get_text_from_table(
    table: List[List[Optional["PDFToken"]]], strip_text: bool = True
) -> List[List[str]]:
    """
    Given a table (of PDFTokens or None), returns a table (of token.text() or '').

    Args:
        table: The table (a list of lists of PDFTokens).
        strip_text (bool, optional): Whether to strip the text for each token of the
                table. Default: True.
    ...
    for row in table:
        new_row = [
            token.Text(strip_text) if token is not None else "" for token in row
        ]
        new_table.append(new_row)
    return new_table
