...
def scale_pdf(k, factors):
    ...
    factors : tuple
        ...
        first two tokens are scaling factors and pdf_y is height of
        pdf.
    Returns
    -------
    knew : tuple
...
def scale_image(tables, v_segments, h_segments, factors):
    ...
    factors : tuple
        ...
        first two tokens are scaling factors and img_y is height of
        image.
    Returns
    -------
    tables_new : dict
...
def overlap(texlines, rectangle):
        for tl in texlines:
            if isRectangleOverlap(tl, rectangle):
                return True
        return False

def nb_overlap(texlines, rectangle):
        overlaps=0
        y0=0
        y1=0
        intersections={}
        ...
        if not intersections:
            return 0
        return max(intersections.values())
...
def get_fonts(textline):
    ...
    for c in textline._objs:
        if isinstance(c, LTChar):
            font_name.append(c.fontname)
            # color = c.graphicstate.ncolor
            # if color is None:
            #     color = c.graphicstate.scolor
            # if isinstance(color, tuple):
            #     if len(color) == 3:
            #         color = [int(255 * col) for col in color]
            #         font_color.append(get_color_name(color, exact=False)['name'])
            #     else:
            #         font_color.append('wrong color code')

            font_size.append(round(c.size, 0))
            font_width.append(c.width)
            font_height.append(c.height)
            if 'Bold' in c.fontname:
                is_bold.append(True)
            else:
                is_bold.append(False)
    ...
    return {'size': max(set(font_size), key=font_size.count),
            'name': max(set(font_name), key=font_name.count),
            'bold': max(set(is_bold), key=is_bold.count),
            'visible': visible,
            'width': max(set(font_width), key=font_width.count),
            'height': max(set(font_height), key=font_height.count),
            'color': max(set(font_color), key=font_color.count) if len(font_color) > 0 else None}

def fix_sentences(texts):
    if len(texts)==0:
        return texts
    returned_strings=[texts[0]]
    for text in texts[1:]:
        if text[0].islower() and returned_strings[-1][-1] != '.':
            returned_strings[-1]+=' '+text
        else:
            returned_strings.append(text)
    return '\n'.join(returned_strings)

def get_page_parameters(LTPage):
...
def get_fonts_statistics(texlines):
    ...
    sizes = [span.font['size'] for line in texlines for span in line.tokens]
    font_names = [span.font['name'] for line in texlines for span in line.tokens]
    colors = [span.font['color'] for line in texlines for span in line.tokens]
    line_spacings_above = [round(line.space_above, 0) for line in texlines if line.space_above>0]
    line_spacings_below = [round(line.space_below, 0) for line in texlines if line.space_below>0]
    font_sizes_by_frequency=sorted(set(sizes), key = lambda ele: -sizes.count(ele))

    return {
        'frequent_size': max(set(sizes), key=sizes.count),
        'largest_size': max(sizes),
        'average_size': mean(sizes),
        'all_sizes_by_frequency': font_sizes_by_frequency,
        'second_largest_size': sorted(font_sizes_by_frequency)[-2] if len(font_sizes_by_frequency)>1 else -1,
        'largest_line_spacings_above': max(line_spacings_above),
        'frequent_line_spacings_above': max(set(line_spacings_above), key=line_spacings_below.count),
        'average_line_spacings_above': mean(line_spacings_above),
        'largest_line_spacings_below': max(line_spacings_below),
        'average_line_spacings_below': mean(line_spacings_below),
        'frequent_line_spacings_below': max(set(line_spacings_below), key=line_spacings_below.count),
...
#                fix_textline_coordinates(obj)
                if ltype == "vertical_text":
                    if isinstance(obj._objs[0], LTChar):
                        if obj._objs[0].matrix[0:4]==(0.0, 1.0, -1.0, 0.0) or obj._objs[0].matrix[0:4]==(0.0, -1.0, 1.0, 0.0):
                            t.append(obj)
                elif ltype == "horizontal_text":
                    if isinstance(obj._objs[0], LTChar) and (obj._objs[0].matrix[0:4] != (0.0, 1.0, -1.0, 0.0) and obj._objs[0].matrix[0:4] != (0.0, -1.0, 1.0, 0.0)):
                        t.extend(fix_text_objects(obj))
                else:
                    t.append(obj)
            else:
...
#trimes spaces before and after words and split string if the numner of spaces between words is greater than nb_spaces
def fix_text_objects(textline, nb_spaces=2):
    #trim spaces at start and end of the words:
    while not isinstance(textline._objs[0], LTAnno) and textline._objs[0]._text==' ':
        del(textline._objs[0])
    while not isinstance(textline._objs[-1], LTAnno) and textline._objs[-1]._text==' ':
        del(textline._objs[-1])

    if not isinstance(textline, LTTextLineHorizontal):
        return [textline]

    texts=""
    sub_textline=LTTextLineHorizontal(word_margin=0.1)
    return_textlines=[]

    for char in textline._objs:
        texts+=char._text
        if len(texts)<nb_spaces or texts[-2:] != nb_spaces*' ':
            if isinstance(char, LTChar):
                sub_textline.add(char)

        elif len(sub_textline._objs)>0:
            while sub_textline._objs[-1]._text==' ':
                del(sub_textline._objs[-1])
            if len(sub_textline._objs)>0:
                return_textlines.append(sub_textline)
            sub_textline = LTTextLineVertical(word_margin=0.1)
    if len(sub_textline._objs) > 0:
        return_textlines.append(sub_textline)

    return return_textlines

#often there are spaces and and invisible annotation before the word.
#this does not give the exact coordinates of the text.
#this function fixes this problem
def fix_textline_coordinates(text_line):
    if isinstance(text_line, LTTextLineHorizontal):

        for char in text_line._objs:
            if char._text.strip()=='':
                continue
            text_line.x0=char.x0
            break
        for char in reversed(text_line._objs):
            if char._text.strip()=='':
                continue
            text_line.x1=char.x1
            break
    return text_line
def get_graphical_object(layout, ltype="line", t=None):
    if ltype == "line":
        LTObject = LTLine
        ...
        for obj in layout._objs:
            if ltype == "line":
                if isinstance(obj, LTLine):
                    ...
                    t += get_graphical_object(obj)
            else:
                if isinstance(obj, LTObject):
                    t.append(obj)
                else:
                    t += get_graphical_object(obj, ltype=ltype)
    except AttributeError:
        pass
    return t

def get_all_objects(layout, t=None):
    """Recursively parses pdf layout to get a list of
    PDFMiner text objects.
    Parameters
    ----------
...
def get_object_type(obj):
    ...
    if isinstance(obj, LTLine):
        ...
        return 'curve'
    elif isinstance(obj, LTTextLineHorizontal):
        return 'textline'
    else:
        return None
...
def has_ovelaping_rectangle(retangle, retangles):
    for rect in retangles:
        if isRectangleOverlap(rect, retangle):
            return True
        return False
...
def segement_overlap(min1, max1, min2, max2):
    return max(0, min(max1, max2) - max(min1, min2))

def get_bounding_object(object, objects):
    for o in objects:
        if o == object:
            continue
        if isInside(object, o):
            return o
    return None
