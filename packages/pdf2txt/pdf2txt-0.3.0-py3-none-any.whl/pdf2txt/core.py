...
import numpy as np
import pandas as pd
from pdf2txt.utils import overlap, nb_overlap
...
#                te.is_valid = False
            if te.is_valid:
                if nb_overlap(lines, te)>=2 or overlap([span for tl in textlines for span in tl.spans], te):
                    te.is_valid=False
        ...
        vertial_splitters=[]
        #extend the separators upward and downlward
        for line_edge in relevant_textedges:
            found=False
            while not overlap([span for tl in textlines for span in tl.spans], line_edge) and line_edge.y0>0 and not overlap(lines, line_edge):
                line_edge.y0-=5
                found=True
            if found:
                line_edge.y0+=15
            ...
            found=False
            while not overlap([span for tl in textlines for span in tl.spans], line_edge) and line_edge.y1 < self.page_height and not overlap(lines, line_edge):
                line_edge.y1 += 5
                found=True
            if found:
                line_edge.y1 -= 15
        ...
        frst_col_x = None
        midle_col_detected=False
        Separator=namedtuple('Separator', ['x0', 'y0', 'x1', 'y1'])
        for line_edge in relevant_textedges:
            #typically first column will be third of the page
            if not midle_col_detected and (np.isclose(line_edge.x0 , self.left_margin+self.page_width/3, atol=25) or np.isclose(line_edge.x0 , self.left_margin+2*self.page_width/3, atol=25)):
                vertial_splitters.append(Separator(line_edge.x0, line_edge.y0, line_edge.x0, line_edge.y1))
                frst_col_x=line_edge.x
                continue
            #if the third of page column is detected then than the midle column would be at the half of the distance remaining
            if frst_col_x and np.isclose(line_edge.x0 ,self.left_margin+(self.page_width+frst_col_x+1)/2.0, atol=20):
                vertial_splitters.append(Separator(line_edge.x0, line_edge.y0, line_edge.x0, line_edge.y1))
