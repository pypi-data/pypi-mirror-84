...
from pdf2txt.parser.base_parser import BaseParser
from pdf2txt.core import Table
...
from pdf2txt.image_processing import (
    adaptive_threshold,
    find_lines,
    find_contours,
    find_joints,
)
from pdf2txt.pdf_parser import parse_pdf
...
class Lattice(BaseParser):
    ...
    def _generate_image(self):
        ...
        pages = convert_from_path(self.filename, dpi=300)
        for page in pages:
            page.save(self.imagename, 'PNG')
    ...
    def extract_tables(self, filename, suppress_stdout=False, layout_kwargs={}):
        ...
        _content=parse_pdf(self.filename)
        _tables = []
        ...
        last_line_id=0
        structured_page={}
        table_bnoxes=sorted(self.table_bbox.keys(), key=lambda x: x[1], reverse=True)

        for table_idx in range(0, len(table_bnoxes)):
            tk=table_bnoxes[table_idx]
            next_table_limit=0
            if table_idx+1<len(table_bnoxes):
                next_table_limit=table_bnoxes[table_idx+1][3]
            for i in range(last_line_id, len(_content)):
                line=_content[i]
                if line[2] > tk[3]:
                    section.append((line[5], line[6]))
                    last_line_id+=1
            ...
            for i in range(last_line_id, len(_content)):
                line=_content[i]
                if line[2] > tk[1] and line[1] > tk[2]:
                    section.append((line[5], line[6]))
                    last_line_id =i+1
            if len(section)>0:
                structured_page["section-"+str(section_id)]=section
                section_id+=1
            ...
            structured_page["section-"+str(section_id)]=table
            section_id+=1
            section=[]
            for i in range(last_line_id, len(_content)):
                line=_content[i]
                if line[2] < tk[1] and line[2]>= next_table_limit:
                    section.append((line[5], line[6]))
                    last_line_id=i+1
            structured_page["section-"+str(section_id)]=section
            section_id+=1
        ...
        if len(table_bnoxes)==0:
            structured_page["section-0"]=[(c[5], c[6]) for c in _content]
