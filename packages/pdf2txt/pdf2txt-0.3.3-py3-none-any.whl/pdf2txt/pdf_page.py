...
from pdf2txt.pdf_objects import *
from pdf2txt.utils import are_in_same_Line, get_fonts_statistics, get_bounding_object, has_ovelaping_rectangle, overlap, nb_overlap
from pdf2txt.paragraph import extract
from pdf2txt.parser.stream import Stream
import copy

Area = namedtuple('Area', ['x0', 'y0', 'x1', 'y1'])


class PDFPage(Stream):
    def __init__(self, parent, layout, page_number, edge_tol=50, **kwargs):
        super().__init__(edge_tol, **kwargs)
        self.layout = layout
        self.parent = parent
    ...
    def _init_layout(self):
        ...
        self.pdf_width, self.pdf_height, self.margin_left, self.margin_right, self.margin_bottom, self.margin_top = get_page_parameters(
            self.layout)
    ...
    def _find_line_structure(self, textlines):
        ...
        while i < len(textlines) - 1:
            ...
            # if not self._is_visible(text):
            #     i += 1
            #     continue

            next_text = Span(textlines[i + 1])
            # while not self._is_visible(next_text) and i < len(textlines)-2 :
            #     i += 1
            #     next_text = Span(textlines[i + 1])
    ...
    def _page_detection_algorithm(self):
        ...
        textlines=[span for tl in self.horizental_text_lines for span in tl.spans]
    ...
    def _get_region_lines(self, region):
        ...
        for gline in self.graphical_lines:
            if gline.width > 5.0 and isRectangleOverlap(gline, region):
                region_lines.append(['line', gline])
...
#            print(table)
            ...
            if self.number == 1:
                ...
                if index_of_line_with_max_size >= 3 and index_of_line_with_max_size <= 5:
                    NB_LINES_IN_HEADER = index_of_line_with_max_size + 1

            # find document title or page header
            line_analyzed = 0
            title_font_cut_off = self.parent.font_statistics['second_largest_size']
            if len(self.font_statistics['all_sizes_by_frequency']) == 3:
                title_font_cut_off = self.parent.font_statistics['largest_size']  # -0.1 #as we are comparing
            ...
            if region_number == 0 and self.font_statistics['second_largest_size'] > 0:  # if second largest font doens not exist (-1), then there is only one font size in the page --> no title or sub title in this page:
                ...
                for i, line in enumerate(candidates_title):
                    ...
                    if line[1].spans[0].font['size']==title_font_cut_off or (line[1].spans[0].font['size'] >= title_font_cut_off and line[1].space_below > line[1].space_above):
                        currentLine += 1
                        if self.number == 1:
                            self.parent.title.append(line[1].spans[0])
                        elif line[0] != "line":
                            self.header.append(line[1].spans[0])

                #look for paragraphs
                remaininglines=[l[1] for l in region_lines[currentLine:] if l[0] == "text"]
                if len (remaininglines):
                    paragraphs.extend(extract(remaininglines))
            else:
        ...
        for p in paragraphs:
            ...
            elif len(p) > 2 and ( p[0].spans[0].font['height'] > p[1].spans[0].font['height'] or p[0].spans[0].font['bold']):
                paragraph.title = p[0]
                paragraph_font = p[0].spans[0].font['name'] + '+size=' + str(p[0].spans[0].font['height'])

                i=1
            last_line_font_size=100
            for l in p[i:-1]:
                paragraph.lines.append(l)
                last_line_font_size=l.spans[0].font['height']
            if p[-1].spans[0].font['height'] > last_line_font_size:
                next_paragraph_tile = p[-1]
            else:
                paragraph.lines.append(p[-1])
