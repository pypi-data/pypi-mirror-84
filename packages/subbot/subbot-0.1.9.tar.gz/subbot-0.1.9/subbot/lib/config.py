...
class SubbotConfig(Config):
    ...
    def check(self):
        if not self["properties_path"]:
            ...
            self.config["properties_hash"] = get_hash(properties_path / "mkv_properties.json")
        ...
        if not self["secrets_path"]:
            print("Select the directory with your upload secrets:")
            self.config["secrets_path"] = ask_directory()
