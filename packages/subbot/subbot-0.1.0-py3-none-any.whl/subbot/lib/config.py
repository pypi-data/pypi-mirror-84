...
class Config():
    def __init__(self, path=None, object_pairs_hook=defaultdict_factory):
        if not path:
            path = Path.cwd()
        self.path = path
        self.config = read_json_dict(path, object_pairs_hook=object_pairs_hook, return_=PathDict)
...
class SubbotConfig(Config):
    ...
    def check(self):
        ...
        if not properties_path:
            print(f"Select the directory where you want to store the MKV properties file or have it stored:")
            properties_path = ask_directory()
            self.config["properties_path"] = properties_path
    ...
    def check_path(self, path):
        ...
        if not path.exists():
            if not (local_path := self.config.get("path_index", {}).get(path, {}).get(hostname, '')):
