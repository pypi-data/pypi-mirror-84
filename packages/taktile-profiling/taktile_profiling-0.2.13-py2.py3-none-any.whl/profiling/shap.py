...
class ShapExplainer(Explainer):
    def __init__(self, func, X, profile_columns=None):
        ...
        if len(X.columns) > len(profile_columns):

            def func_fused(X_profile_columns, X_background):
                X_combined = X_profile_columns.copy()
                for col in X_background.columns:
                    X_combined[col] = X_background[col].iloc[[0]]
                X_combined = X_combined.reindex(columns=X.columns)
                return func(X_combined)

            X_background = self._create_background_data(X.drop(columns=profile_columns))
            self.func = partial(func_fused, X_background=X_background)
            self.X = self._create_background_data(X[profile_columns])
            self.n_col = len(profile_columns)

        else:
            self.func = func
            self.X = self._create_background_data(X)
            self.n_col = len(X.columns)
