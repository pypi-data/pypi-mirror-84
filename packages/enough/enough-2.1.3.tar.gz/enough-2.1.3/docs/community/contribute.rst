...
Use ``--log-cli-level`` switch in order to increase the test log level,
following switches are `pytest ones <https://docs.pytest.org/en/stable/contents.html>`_:

* ``tests/run-tests.sh tox -e py3 -- --log-cli-level=DEBUG -s -x tests/enough/common/test_openstack.py``
