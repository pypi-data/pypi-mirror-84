...
class DicomReaderWriter:
    ...
    def add_RT(self, PathDicom):
        RT_Files = [os.path.join(PathDicom, i) for i in os.listdir(PathDicom) if i.find('.dcm') != -1]
        reader = sitk.ImageFileReader()
        for lstRSFile in RT_Files:
            reader.SetFileName(lstRSFile)
            try:
    ...
    def contours_to_mask(self, index):
        ...
        for i in range(len(Contour_data)):
            ...
            mask_indexes = np.where((self.row_val > 0) & (self.row_val < self.image_size_rows) &
                                    (self.col_val > 0) & (self.col_val < self.image_size_cols))[0]
            mask_rows = self.row_val[mask_indexes]
            mask_cols = self.col_val[mask_indexes]
            temp_mask[mask_rows, mask_cols] = 0
