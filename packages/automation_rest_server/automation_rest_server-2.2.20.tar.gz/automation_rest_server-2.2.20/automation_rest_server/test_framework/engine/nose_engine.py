...
class NoseEngine(object):
    ...
    def command_run_test(self, test_case, test_path):
        ...
        command_line = "nosetests --exe --nocapture --with-xunit --xunit-file={} -x {}"\
            .format(xml_path, test_path)
        child1 = subprocess.Popen(command_line, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        std_out, std_err = child1.communicate()
        msg = self.get_msg(std_out, std_err)
        log_path = self.save_msg(msg, test_case)
        ret = True if child1.returncode == 0 else False
        return ret, log_path

    def run(self, test_case, test_path, parameters, queue):
        ret, log_path = self.command_run_test(test_case, test_path)
        if ret is True:
            log.INFO("TestCase run succeed.%s", test_case)
        else:
            log.ERR("TestCase run failed. %s", test_case)
        result = {"name":test_case, "result": ret, "log_path": log_path}
        queue.put(result)
        return ret
