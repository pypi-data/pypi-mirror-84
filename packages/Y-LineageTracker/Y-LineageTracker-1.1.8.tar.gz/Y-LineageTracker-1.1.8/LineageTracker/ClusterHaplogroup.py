...
class ClusterAnalysis(object):
    ...
    def _calculate_MDS_fst(self, population_count, fst_output):
        ...
        pops = sorted(population_count.index.tolist())
    ...
    def output_cluster(self, method, freq, output_set, hap_data, population_data):
        ...
        # x and y labels
        if method == 'pca':
            plt.xlabel('PC1  %.2f%%'% (pca.explained_variance_ratio_[0]*100), fontsize=16)
            plt.ylabel('PC2  %.2f%%'% (pca.explained_variance_ratio_[1]*100), fontsize=16)
        elif method == 'mds':
        ...
        if 'label_list' in vars():
            ...
            plt.legend(frameon=True, fontsize=12, markerscale=0.9, shadow=True, edgecolor='dimgrey', loc='best')
        else:
            for i in range(population_data.index.size):
        ...
        if population_freq.index.size < 40:
            texts = [plt.text(x=coordinates[i, 0]+0.008, y=coordinates[i, 1]-0.01, s=population_freq.index.tolist()[i]) for i in range(len(matrix))]
            import warnings
            warnings.filterwarnings('ignore')
            adjust_text(texts)
        else:
            self.logger.warning('[Y-LineageTracker] Too much population in population file')
