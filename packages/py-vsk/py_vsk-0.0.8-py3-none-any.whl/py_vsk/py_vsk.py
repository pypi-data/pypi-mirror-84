...
def vsk_mle(x):
  ...
  Parameters:
    x : A numeric vector in the interval of (0, 1), which can be a list, 
        1-D numpy array, or pandas series, that is assumed the Vasicek 
        distribution.
  Returns:
...
def get_Rho(x, P, Alpha):
  """
  The function solves for Rho such that vsk_cdf(x, Rho, P) = Alpha.
  Parameters:
    x     : A numeric scalar representing the ppf of Vasicek that the
            Rho parameter is solved for.
    P     : The P parameter in the Vasicek distribution
    Alpha : A numeric scalar representating the percentile
  Returns:
    A scalar representing the Rho parameter in the Vasicek distribution
  Example:
    import py_vsk
    x = py_vsk.vsk_ppf([0.99], Rho = 0.2, P = 0.1)[0]['ppf']
    py_vsk.get_Rho(x, P = 0.1, Alpha = 0.99)
    # 0.20000105607863755
  """

  fn = lambda r: abs(vsk_ppf([Alpha], Rho = r, P = P)[0]["ppf"] - x)

  return(minimize_scalar(fn, bounds = (0, 1), method = "bounded").x)
...
def vsk_Rho(x, p):
  """
  The function estimates the Rho parameter in the Vasicek distribution by 
  using MLE by assuming the known P parameter.
  Parameters:
    x : A numeric vector in the interval of (0, 1), which can be a list,
        1-D numpy array, or pandas series, that is assumed the Vasicek 
        distribution.
    p : A numeric vector in the interval of (0, 1), which can be a list,
        1-D numpy array, or pandas series. Each value in p can be a constant
        or varying. p and x should have the same lengths.
  Returns:
    A scalar representing the Rho parameter in the Vasicek distribution.
  Example:
    import py_vsk, numpy
    x = py_vsk.vsk_rvs(100, Rho = 0.2, P = 0.1) 
    p = [numpy.mean(x)] * len(x)
    py_vsk.vsk_Rho(x, p)
    # 0.16374740746144395
  """
  ...
  _p = [_ for _ in p if _ > 0 and _ < 1 and not numpy.isnan(_)]

  if len(_x) == len(_p):
    _d = list(zip(_x, _p))
  else:
    print("x and p have different lengths!")
    return

  fn = lambda r: -sum(numpy.log([vsk_pdf([_[0]], Rho = r, P = _[1])[0]['pdf'] 
                                 for _ in _d]))
