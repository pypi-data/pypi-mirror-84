...
def create_azure(ctx, name, resource_group_name, service_principal_name):
    ...
    with api_client_exception_handler():
        ...
        cluster_utils.echo_delimiter()
        web_url = ctx.obj["web_url"]
        owner = ctx.obj["owner"]
        name = cluster["name"]
        url = f"{web_url}/{owner}/clusters/{name}"
        click.echo(
...
def create_service_principal(client, service_principal_name, name):
    ...
    for i in range(NUM_RETRIES):
        try:
            ...
            break
    click.echo(f"Service Principal `{service_principal_name}` Created!")
...
def create_storage_account(storage_client, cluster_name, resource_group_name, params):
    ...
    click.echo(
        f"Storage account `{storage_account_name}` under resource group `{resource_group_name}` created!"
    )
...
def add_az_bucket(spell_client, cluster, account_name):
    ...
    with api_client_exception_handler():
        spell_client.add_bucket(account_name, cluster_name, "azblob", storage_key)
    click.echo(f"Storage account {account_name} has been added to cluster {cluster_name}!")
