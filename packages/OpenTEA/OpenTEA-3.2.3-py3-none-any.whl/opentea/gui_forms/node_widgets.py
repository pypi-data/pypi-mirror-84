...
from tkinter import Entry, Label, Frame, StringVar, LEFT, filedialog
...
class OTContainerWidget(OTNodeWidget):
    ...
    def __init__(
        self, schema, root_frame, name, tab, n_width=1, relief="ridge"
    ):
        """Startup class.
        ...
        self._holder.pack(side="top", fill="x", padx=0, pady=0, expand=False)
        ...
        self.head.pack(side="top", fill="x", padx=2, pady=2, expand=False)
...
class OTMultipleWidget:
    ...
    def rename_callback(self, item_name):
        ...
        trans_frame.place(
            x=bbox[0] - 1, y=bbox[1] - 1, width=bbox[2] + 2, height=bbox[3] + 2
        )
    ...
    def load_from_file(self):
        ...
        path = filedialog.askopenfilename(
            title="Partial load from file", filetypes=[("YAML Files", "*.yml")]
        )
...
class OTXorWidget:
    ...
    def update_xor_content(self, name_child, data_in=None):
        ...
        self.tree[name_child] = OTContainerWidget(
            child_schema, self._xor_holder, name_child, self.tab, relief="flat"
        )
        if data_in is None:
