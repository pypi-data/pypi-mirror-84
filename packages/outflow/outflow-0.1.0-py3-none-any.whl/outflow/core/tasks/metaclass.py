...
from outflow.core.generic.types import TargetOutputs
...
class TaskMeta(type):
    def __new__(cls, name, bases, dct, *args, **kwargs):
        ...
        dct["with_context"] = dct.get("with_context", True)
    ...
    def __init__(self, name, bases, dct):
        ...
        for index, input_arg_name in enumerate(full_args_spec.args):
            if self.with_context and index == 0:
                ...
                continue
            self.add_input(
                Target.input(identifier=input_arg_name), identifier=input_arg_name
            )
