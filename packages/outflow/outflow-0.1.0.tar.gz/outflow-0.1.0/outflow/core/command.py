...
class Command(Group, metaclass=Singleton):
    ...
    def __init__(self, *args, invokable=True, on_before=None, **kwargs):
        ...
        DeclicCommand.__init__(
            self, *args, name=to_snake_case(self.__class__.__name__), **kwargs,
        )
        self.add_arguments()
    ...
    def invoke(self, args, main_actor, extra_args):
        """
        ...
        return main_actor.run(task_list=task_list)

    def main(self, argv=None, main_actor=None):
        args, extra_args = self.parser.parse_known_args(argv)

        # update the pipeline context with the CLI args
        main_actor.update_pipeline_context_args(args)
        return args.__declic_invoke_function__(
            args, main_actor=main_actor, extra_args=extra_args
        )

    def __call__(self, argv=None, main_actor=None):
        return self.main(argv, main_actor=main_actor)
    ...
    def subcommand(cls, *args, name=None, **kwargs):
        ...
        with_pipeline_context = kwargs.pop("with_task_context", False)
        ...
        def decorator(callback_or_command_class):
            ...
            if is_class:
                ...
                def setup_tasks(self):
                    ...
                    @Task.as_task(
                        name=subclass_name, with_context=with_pipeline_context
                    )
