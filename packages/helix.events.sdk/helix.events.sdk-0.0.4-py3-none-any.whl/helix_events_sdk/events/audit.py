...
class AuditEvent(CloudEvent):
    def __init__(self, source: Source, data: Audit = None):
        attributes = {
        ...
        super().__init__(attributes=attributes, data=fhir_audit_event.as_json())

    def _get_fhir_audit_event(self, data):
        ...
        fhir_audit_event.agent = [user_agent, patient_agent]
    ...
    def _get_audit_event_entity(self, data):
        entity = AuditEventEntity()
        entity.name = data.document_type
        return entity

    def _get_audit_event_source(self, data):
    ...
    def _get_patient_agent(self, data):
    ...
    def _get_user_agent(self, data):
        ...
        user_agent.network = AuditEventAgentNetwork(jsondict={"address": data.ip_address})
        user_agent.role = [CodeableConcept(jsondict={"coding": [{
            "code": data.user_role
        }],
            "text": data.user_role})]
        return user_agent
