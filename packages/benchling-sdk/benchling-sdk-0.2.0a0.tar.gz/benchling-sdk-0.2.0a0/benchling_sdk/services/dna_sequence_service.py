...
class OnRegistrationFailure(Protocol):
    def __call__(
        self, service: "DnaSequenceService", created_sequence: DnaSequence, registration_task: AsyncTask
    ) -> None:
        ...


def _default_on_registration_failure(
    service: "DnaSequenceService", created_sequence: DnaSequence, registration_task: AsyncTask
) -> None:
    service.archive(dna_sequence_ids=[created_sequence.id], reason=Reason1.OTHER)
...
class DnaSequenceService(BaseService):
    ...
    def create_and_register(
        self,
        dna_sequence: DnaSequenceCreate,
        registry_id: str,
        on_failed_registration: OnRegistrationFailure = _default_on_registration_failure,
    ) -> DnaSequence:
        created_sequence = self.create(dna_sequence)
        task = self._registry_service.register(entity_ids=[created_sequence.id], registry_id=registry_id)
        ...
        if completed_task.status != Status1.SUCCEEDED:
            on_failed_registration(
                service=self, created_sequence=created_sequence, registration_task=completed_task
            )
            raise RegistrationError.from_task(completed_task)
        registered_sequence = self.get_by_id(created_sequence.id)
