...
from localstack_ext.utils.common import download_github_artifact
...
def install_postgres():
 if not in_docker():
  return
 try:
  run('which postgres',print_error=SReVy)
 except SReVG:
  install_alpine_package('postgresql','RDS')
 try:
  run('which pg_config',print_error=SReVy)
 except SReVG:
  install_alpine_package('postgresql-dev','RDS')
def install_redis():
 try:
  run('which redis-server',print_error=SReVy)
 except SReVG:
  install_alpine_package('redis','ElastiCache')
 return 'redis-server'
def install_mqtt():
 try:
  run('which mosquitto',print_error=SReVy)
 except SReVG:
