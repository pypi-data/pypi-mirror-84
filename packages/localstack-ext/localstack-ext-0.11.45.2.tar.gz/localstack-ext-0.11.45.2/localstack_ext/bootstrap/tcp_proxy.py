...
class AttrDict(ckiNq):
 def __init__(self,*args,**kwargs):
  ckiNg(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and ckiNt(socket,'IPV6_V6ONLY'):
...
class Half(ckiNr):
 def __init__(self,opt,sock,addr,ckiNW):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.ckiNW=ckiNW
  self.name='peer' if self.ckiNW else 'client'
  self.queue=[]
  self.dest=ckiNv
  self.err=ckiNv
  self.ready=ckiNU
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':ckiNU,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':ckiNU,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':ckiNU,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':ckiNU,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
