...
class FourierCoefficients(VMECBase):
    ...
    def get_indices(
        ...
        coeffs = np.arange(radial * modes ** 2 * 2)
        ...
        indeces = [
            coeffs[s, m * (2 * modes + 1) + n]
            for s in radial_points
            for m in poloidal_modes
            for n in toroidal_modes
            if not (m == 0 and n < 0)
        ]

        return indeces

    def extract_dimensions(coeffs):
        """Retrieve number of radial points, and inferred fuorier modes.

        Args:
            coeffs (numpy.ndarray): input coefficients array.

        Returns:
            tuple: number of radial points and fourier modes.
        """

        return coeffs.shape[0], int(np.sqrt(coeffs.shape[1] / 2))
    ...
    def from_array(cls, coefficients):
        ...
        radial_points, fourier_modes = FourierCoefficients.extract_dimensions(
            coefficients
        )
        poloidal_modes = list(range(fourier_modes))
        toroidal_modes = list(range(-fourier_modes, fourier_modes + 1))

        #  Extend coefficients for m=0, n<0
        if len(poloidal_modes) * len(toroidal_modes) != coefficients.shape[1]:
