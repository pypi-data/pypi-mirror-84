...
def _add_estimed_language(grph, node):
    """On analyse le process, on lit les parametres
    On peut pas le faire dans le bin.
    On analysie la ligne de commande selon le langage.

    La ligne de commande pourrait etre un objet car on le trouve:
    - Dans les processes.
    - Dans les makefiles, *.proj, *.vcxproj .

    Ca contient des literals avec une clef de tri.
    Les literaux sont heterogenes: Chaines, fichiers
    Comment trier ? Ajouter une valeur, une variable CGI, a la propriete ?

    Peut-on utiliser des BNodes pour trier

    """
    grph.add((node, lib_common.MakeProp("Esimted language"), rdflib.Literal("tralala")))
...
def AddInfo(grph, node, entity_ids_arr):
    ...
    try:
        ...
        # Parser intelligemment la ligne de commande ? On pourrait creer du HTML ligitime comme du XML
        ...
        _add_estimed_language(grph, node)
        grph.add((node, lib_common.MakeProp("Esimted language"), rdflib.Literal("tralala")))
