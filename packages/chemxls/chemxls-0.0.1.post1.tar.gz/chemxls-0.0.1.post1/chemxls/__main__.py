	...
	def __init__(self,file_content,fragment_link_one=False):
		...
		for i,a in enumerate(a.strip() for a in self.list):
			...
			if a[0].isdigit() or a[0].islower():                                                                continue
				...
				elif a.startswith(text_d):
					pass;                                                       self.apt_charge_idxs.append(i); continue
				...
				if a.startswith("Link1:"):
					...
					#cond_1 = self.raw_route_keys and "opt" in self.raw_route_keys
					#cond_2 = self.raw_route_keys and "freq" in self.raw_route_keys
					#cond_3 = 1 if cond_1 and cond_2 else 0
					#if cond_3 and len(self.linked_job_idxs) > cond_3:
...
class Var:
	...
	def __init__(self,conf_file=conf_file):
		...
		self.options = [
			...
			{"short":"HOMO",               "uid":"025", "extension":a[4], "supl":False, "float":True , "hyp":False, "long":"HOMO from Filename.log"                                     },
			{"short":"LUMO",               "uid":"026", "extension":a[4], "supl":False, "float":True , "hyp":False, "long":"LUMO from Filename.log"                                     },
			{"short":"HOMO-LUMO",          "uid":"027", "extension":a[4], "supl":False, "float":True , "hyp":False, "long":"HOMO-LUMO gap from Filename.log"                            },
...
class FileFolderSelection(tk.Frame):
	def __init__(self,parent):
		...
		check_button = tk.Checkbutton(box, text="Auto",
									  ...
									  command=lambda self=self:check_lock(self.auto_set_supl))
		check_button.grid(column=2, row=0, sticky="w")
		self.check_buttons.append(check_button)
		# XLS file
		...
		check_button = tk.Checkbutton(box, text="Auto",
									  ...
									  onvalue=True, offvalue=False,
									  command=lambda self=self:check_lock(self.auto_set_xls()))
		check_button.grid(column=2, row=0, sticky="w")
		self.check_buttons.append(check_button)
	...
	def set_in_folder(self):
		...
		if type(in_folder) == str and in_folder.strip() != "":
			...
			self.in_folder_label.set(trim_str(self.in_folder,self.str_width))
			if self.xls_path_auto.get(): check_lock(self.auto_set_xls())
			if self.supl_folder_auto.get(): check_lock(self.auto_set_supl())
		else:
...
class ListBoxFrame(tk.Frame):
	def __init__(self,parent):
		...
		self.populate_a("any")
		self.listbox_a.grid(column=0, row=1,rowspan=2,sticky="news")
		scrollbar = tk.Scrollbar(self, orient="vertical")
		scrollbar.config(command=self.listbox_a.yview)
		scrollbar.grid(column=1, row=1, rowspan=2, sticky="ns")
		...
		#BOTTOM LEFT PANEL
		frame = tk.Frame(self)
		frame.grid(column=0,row=3,columnspan=2)
		...
		self.listbox_b.grid(column=3, row=1, rowspan=2, sticky="news")
		self.populate_b(self.preferences.startup_options)
		scrollbar = tk.Scrollbar(self, orient="vertical")
		scrollbar.config(command=self.listbox_b.yview)
		scrollbar.grid(column=4, row=1,rowspan=2,sticky="ns")
		self.listbox_b.config(yscrollcommand=scrollbar.set)

		#BOTTOM RIGHT PANEL
		frame = tk.Frame(self)
		frame.grid(column=3,row=3,columnspan=2,sticky="news")
		...
		frame = tk.Frame(self)
		frame.grid(column=3,row=5,columnspan=2,rowspan=1,sticky="news")
		...
		self.progress.grid(column=0,row=6,columnspan=6,sticky="news",pady="5")
		...
		label.grid(column=0, row=7,columnspan=6,sticky="e")
		self.path_sheet = ["abs",
	...
	def delete_selection(self):
		self.listbox_b.delete(tk.ANCHOR)
	@check_lock
	def move_right(self):
		...
		if (ext:=".log") in extensions:
			...
			if other_log_properties and is_file:
				...
				for i,b in enumerate(logs):
					...
					if (n:="E_ZPE"            )in x: up({n: b.thermal[4] if b.thermal[4] else "-"})
					if (n:="E_tot"            )in x: up({n: b.thermal[5] if b.thermal[5] else "-"})
					if (n:="H_corr"           )in x: up({n: b.thermal[6] if b.thermal[6] else "-"})
					if (n:="G_corr"           )in x: up({n: b.thermal[7] if b.thermal[7] else "-"})
					if (n:="E0+E_ZPE"         )in x: up({n: b.thermal[0] if b.thermal[0] else "-"})
					if (n:="E0+E_tot"         )in x: up({n: b.thermal[1] if b.thermal[1] else "-"})
					if (n:="E0+H_corr"        )in x: up({n: b.thermal[2] if b.thermal[2] else "-"})
					if (n:="E0+G_corr"        )in x: up({n: b.thermal[3] if b.thermal[3] else "-"})
					if (n:="Done?"            )in x: up({n: "Yes" if b.normal_termin else "No"})
					if (n:="Error"            )in x: up({n: b.error_msg if b else "-"})
					if (n:="HOMO"             )in x: up({n: b.homo if b.homo else "-"})
					if (n:="LUMO"             )in x: up({n: b.lumo if b.homo else "-"})
					if (n:="HOMO-LUMO"        )in x: up({n: b.homo_lumo_gap if b.homo else "-"})
