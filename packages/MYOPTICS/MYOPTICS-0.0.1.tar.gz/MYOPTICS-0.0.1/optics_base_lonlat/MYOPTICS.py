# from optics_base_lonlat import earth_distance
...
def earth_distance(array1, array2):
    #'''
    #返回单位米，若要返回千米，半径取6371，米用6378137
    #array1, array2分别为两对坐标
    #'''
...
def decide_minsample(df_lon_lat, maxeps, minsample_scale, low, up):
    ...
    for minsamples in range(minsample_scale[0], minsample_scale[1], minsample_scale[2]):
        try:
            clust = OPTICS(min_samples = minsamples, max_eps = maxeps, xi = 0.5, metric = earth_distance).fit(df_lon_lat.iloc[:, :2])
        except ValueError:
            continue
...
class Myoptics:
    ...
    def my_optics(self):
        clust = OPTICS(min_samples = self.minsamples, max_eps = self.maxeps, xi = 0.5, metric = earth_distance).fit(self.data.iloc[:, :2])
...
# 聚类图
def clust_plot(df_lon_lat_labels_reachability_labelsordering, fsize = (16, 8))
    da = df_lon_lat_labels_reachability_labelsordering
    L = len(set(da['labels']))
    total = len(da)
    ratio = len(da[da['labels'] != -1]) / total
    ...
    plt.legend()
    plt.title(str(total) + ',  ' + '{:.2%}'.format(ratio))
    plt.grid()
    plt.show()