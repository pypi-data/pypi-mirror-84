...
class Idcard_Indonesia(): 
    ...
    def __get_address(self, text_list): 
        ...
        for idx, i in enumerate(text_list): 
            if i in ["Alamat"]: 
                address = text_list[idx + 7]
                break
            elif i in ["Kecamatan"]: 
                address = text_list[idx + 1]
                break 
        
        address = address + " " + text_list[1] + " " + text_list[0]
        return address
    ...
    def __get_gender(self, text_list): 
        gender = "-"
        for idx, i in enumerate(text_list): 
            if i in ["Jenis kelamin"]: 
                gender = text_list[idx + 1]
                break 
        if "LAKI" in gender or "LAK" in gender or "AKI" in gender: 
            gender = "M"
        else: 
            gender = "F"
        return gender 
    ...
    def process(self, text_list): 
        name = self.__get_name(text_list)
        num_ic = self.__get_ic(text_list)
        dob = self.__get_dob(text_list)
        gender = self.__get_gender(text_list)
        address = self.__get_address(text_list)
        state = "-"
