...
class BasCifarClassfier(BasObj):
    ...
    def predict_arrays(self, ims, Dataset, print_info=True):
        num_im = len(ims)
        if num_im<self.args.test_batch: return self.predict_arrays_m(ims=ims, print_info=print_info)
        timer1 = Timer()
        testset = Dataset(name=self.args.dber , train=False, data=ims, im_shape=self.args.input_shape, indexing=True)
