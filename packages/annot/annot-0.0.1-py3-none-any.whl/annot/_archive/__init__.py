...
class AbstractAnnotClass(object):
    ...
    def _write(self, s):
        ...
        """
        raise NotYetImplemented('Must be implemented by subcalss')

    def update(self):
        """
        """
        raise NotYetImplemented('Must be implemented by subclass')        
    ...
    def add_dataframe(self, df, df_name,
        ...
        Arguments:
            ...
            df_name (str) - a unique identifier for the dataframe,
                which will be used to save the file.
        ...
        Keyword arguments:
            ...
            See annot_dataframe() for documentation on formatters
        ...
        if not overwrite and os.path.exists(pickle_path):
            raise AnnotException('Path already exists. '
                'Use overwrite=True: {}'.format(pickle_path))
        with open(pickle_path, 'wb') as f:
            pickle.dump(df, f)        
        
        annot_line = '\n\nannot_dataframe("{}"'.format(index_md_path)                                 
        if rows is not None:
            annot_line += ', rows={}'.format(rows)
        if datatables:
            annot_line += ', datatables=True'
        if narrow:
            annot_line += ', narrow=True'
        if not index:
            annot_line += ', index=False'
        if formatters:
            annot_line += ', formatters="{}"'.format(formatters)
        annot_line += ')\n'
        self._write(annot_line)
        self.save()
    ...
    def add_csv(self, csv_path, rows=10, copy=True,
                datatables=False):
        ...
        Keyword arguments:
            rows (int) - Only include this many rows in the markdown
...
class Page(AbstractAnnotClass):
    """A collection of files representing a markdown page with
    csvs and images, with enhanced markdown rules for generating
    the HTML page
    """
    
    def __init__(self, annot_dir=DEFAULT_ANNOT_DIR, overwrite=False):
        """
        Keyword arguments:
        ...
        """The output HTML file that is rendered from markdown
        """

        self._init_sections(use_disk=not overwrite)
    ...
    def render(self, verbose=False):
        ...
        annot_md = (
            '<p><a href="./index.md.txt">Annot source</a> '
            '<a href="./temp/render.md.txt">Markdown source</a></p>')+\
            self.content
    ...
    def clear(self, prompt=True):
        ...
        Arguments:
            prompt (book) - Prompt the user and cancel if they don't
                provide a confirmation to delete.
        
        Returns:
            (None)
        """
        if prompt and input('delete? type ''yes'': ').lower()!='yes':
            return
...
def annot_csv(csv_path, annot_dir, rows=None, datatables=False,
              narrow=False):
    ...
    Keyword arguments:
        rows (int) - number of rows to include, e.g. 10. Used to
            show a sample of the csv to keep the markdown/HTML
            from getting to large.
        datatables (bool: If true, present with DataTables.js
        narrow (bool): If true, use half width
    
    Returns:
        (str) markdown string for first 10 lines of table
    """
    with open(csv_path, 'r') as f:
        # full file
        if not rows:
...
def annot_dataframe(df_pickle_path, annot_dir, rows=None, datatables=False,
    ...
    # load column formatters, if any
    formatters_str=formatters
    formatters={}
    for formatter in formatters_str.split('},'):
        if not formatter:
            continue
        if not formatter.endswith('}'):
            formatter+='}'
        try:
            i=formatter.index('=')
        except ValueError:
            raise ValueError(
                'Formatter string missing equal sign: {}'.format(
                    formatter)) from None        
        col_name=formatter[:i]
        if col_name not in df.columns:
            raise ValueError(
                ("Formatter column ('{}') not in "
                 "dataframe columns: {}").format(
                     col_name, df.column))
        
        col_format=formatter[i+1:]            
        formatters[col_name]=lambda x: col_format.format(x)
    ...
    # is a row of str values.
    table = []
    col_names=list(df.columns)
    if index:
        col_names=list(df.index.names)+col_names
    ...
    for i, df_row in df.iterrows():
        ...
        if not index:
            ...
            row_values=[str(_) for _ in i]
        else:
            row_values=[str(i)]
        # add each row value, with optional formatter
        for col_name, val in df_row.items():
            formatter=formatters.get(col_name, str)
...
def parse_annot_md(line, starts_with):
    ...
    Returns:
        (list, dict) - The first element of the tuple is the list of
           positional arguments. The second element is the dict of
           keyword arguments. 
    """
