    ...
    class FilePath(object):
        ...
        def exists(self, sudo=False):
            if not sudo:
                ...
                try:
                    output = subprocess.check_output(["ls","-ld",self.path]).decode()
                    if "No such file or directory" in output:
                        return False   
                    else: return True
                except subprocess.CalledProccessError: return False
            #
        def mount(self):
...
fp = Formats.FilePath("/tmp/truyhgjsdgjds")
print("EXISTS:",fp.exists(sudo=True))
quit()