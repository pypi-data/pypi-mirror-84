...
class Atom(Doc, Seed):
    ...
    """
    # _REGEX = Name.CLASS_SEPARATOR.join([Zulu._REGEX,
    #                                     Key._REGEX,
    #                                     Identity._REGEX])

    REGEX = r'\d{8}T\d{6}u\d{6}Z\_\_\_[a-z][a-z_0-9]*[a-z0-9]\_\_\_[0-9A-F]{8}\_[0-9A-F]{4}\_[0-9A-F]{4}\_[0-9A-F]{4}\_[0-9A-F]{12}'
    ...
    #         return False
    #     z, k, i = parts
    #     if not Zulu.is_seed(z) \
    #             or not Key.is_seed(k) \
    #             or not Identity.is_seed(i):
    #         return False
    #     return True
