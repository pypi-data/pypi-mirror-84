...
class IORepository(metaclass=Singleton):
    ...
    def save_error(error_msg: str) -> None:
        ...
        GrpcRepository().get_sdk_engine_client().OutputMessage(
            OutputMessageData(message_type=OutputMessageTypes.OMT_Error, message=error_msg)  # type: ignore
        )
    ...
    def save_warn(warn_msg: str) -> None:
        ...
        GrpcRepository().get_sdk_engine_client().OutputMessage(
            OutputMessageData(message_type=OutputMessageTypes.OMT_Warning, message=warn_msg)  # type: ignore
        )
    ...
    def save_info(info_msg: str) -> None:
        ...
        GrpcRepository().get_sdk_engine_client().OutputMessage(
            OutputMessageData(message_type=OutputMessageTypes.OMT_Info, message=info_msg)  # type: ignore
        )
