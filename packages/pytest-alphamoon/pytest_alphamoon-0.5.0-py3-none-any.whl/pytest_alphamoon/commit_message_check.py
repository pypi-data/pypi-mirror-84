...
REGEX = re.compile(
    r'^((?P<alternative_format>Initial commit|Merge branch [\s\S]+?|Revert [\s\S]+?)|'
    r'((?P<header>'
...
def pytest_sessionstart(session):
    ...
    if config.getoption(PLUGIN_NAME):
        config.beginning_commit = config.getini(CHECK_COMMIT_MESSAGES_FROM_INI)
...
class File(pytest.File):
    def collect(self):
        yield CommitMessageItem.from_parent(self)
...
class CommitMessageItem(pytest.Item):
    ...
    def runtest(self):
        try:
            repo = Repo(path=self.config.rootdir)
        except exc.InvalidGitRepositoryError:
            raise CommitMessageError(self.fspath, "Invalid Git repository")

        rev = (f'{self.config.beginning_commit}..{repo.active_branch}'
               if self.config.beginning_commit else None)

        try:
            for commit in repo.iter_commits(rev=rev, paths=self.fspath):
            ...
            raise CommitMessageError(self.fspath, "Invalid beginning commit hash")
        except ValueError as e:
            raise CommitMessageError(self.fspath, str(e))
