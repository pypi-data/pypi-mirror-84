...
class PubsubMessager(messager.Messager):
    ...
    def init_insight(self):
        ...
        topic_list = [t.name for t in publisher.list_topics(request={"project": project_path})]
        for topic_id in self.insight_topics:
    ...
    def init_topic(self, topic_id):
        ...
        project_path = '/'.join(['projects', project_id])
        topic_path = publisher.topic_path(project_id, topic_id)
        if topic_path not in [t.name for t in publisher.list_topics(request={"project": project_path})]:
            topic = publisher.create_topic(request={"name": topic_path})
            logging.info("Topic created: {}".format(topic))
