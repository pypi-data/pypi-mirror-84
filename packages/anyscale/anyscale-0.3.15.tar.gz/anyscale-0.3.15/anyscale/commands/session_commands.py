...
@click.option(
    "-i",
    "--iterative-sync",
    help="Set to true to have autosync sync only the files have changed. This is slightly quicker but does not have support for arbitrary rsync_filter values.",
    is_flag=True,
    default=False,
)
def anyscale_autosync(
    session_name: Optional[str], verbose: bool, sync_git: bool, iterative_sync: bool
) -> None:
    session_controller = SessionController()
    session_controller.autosync(
        session_name, verbose, sync_git, full_sync=not iterative_sync
    )
