...
class SSHD(object):
	...
	def __check_utils_installed__(self, usernames=[]):
		...
		for username in usernames:
			...
			# non existant.
			fp = Formats.FilePath(f"{HOME_BASE}{username}/.ssh/utils/.version.py")
			if not fp.exists(sudo=True): 
			...
			# check version.
				...
				if str(version) != str(Files.File(path=f'{SOURCE_PATH}/{VERSION}/classes/utils/.version.py', load=True).data):
					to_install.append(username)
