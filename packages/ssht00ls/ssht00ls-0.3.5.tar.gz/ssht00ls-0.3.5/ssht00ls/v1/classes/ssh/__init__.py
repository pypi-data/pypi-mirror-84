...
class Config(object):
	...
	def enable_keys(self, username=None, public_keys=[]):
		...
		if sudo: command = ["sudo"]
		output = utils.__execute__(command + ["cat", f"{HOME_BASE}/{username}/.ssh/authorized_keys"], return_format="array")
		print("AUTHORIZED KEYS:  ",output)
		# handle ssh dir not installed.
		try:
			if "No such file or directory" in output[0]:
				print('NO SUCH FILLEEELELELEELE')
				ssh_dir.check(
					directory=True,
					permission=770,
					owner=username,
		...
		for key in public_keys:
			if key.replace("\n", "") not in output:
				auth_keys.permission.set(permission=600, sudo=sudo)
				ssh_dir.permission.set(permission=700, sudo=sudo)
				auth_keys.ownership.set(owner=username, sudo=sudo)
				ssh_dir.ownership.set(owner=username, sudo=sudo)
				key = key.replace("\n", "")
				return r3sponse.error_response(f'Failed to enable public key [{key}].')
	...
	def disable_keys(self, username=None, public_keys=[]):
		...
		for key in public_keys:
			...
			if key in output:
				...
				auth_keys.ownership.set(owner=username, sudo=sudo)
				ssh_dir.ownership.set(owner=username, sudo=sudo)
				return r3sponse.error_response(f'Failed to disable public key [{key}].')
