...
class Config(object):
	...
	def create_alias(self, 
		...
		return response
	def check_keys(self, username=None, public_keys=[]):

		# make readable.
		if username == None: username = OWNER
		sudo = OWNER != username
		sudo_option = ""
		if sudo: sudo_option = "sudo "
		ssh_dir = Formats.FilePath(f"{HOME}/{username}/.ssh/", check_existance=False)
		auth_keys = Formats.FilePath(f"{HOME}/{username}/.ssh/authorized_keys", check_existance=False)
		...
		output = utils.__execute__(command + ["cat", f"{HOME}/{username}/.ssh/authorized_keys"], return_format="array")
		
		# handle ssh dir not installed.
		try:
		...
		for key in public_keys:
			if key.replace("\n", "") not in output:
				auth_keys.permission.set(permission=600, sudo=sudo)
				ssh_dir.permission.set(permission=700, sudo=sudo)
				key = key.replace("\n", "")
				return utils.__error_response__(f'Public key [{key}] is not activated.')
		...
		# success.
		return utils.__success_response__(f'Successfully confirmed that the specfied {len(public_keys)} public key(s) are activated.')
	...
	def enable_keys(self, username=None, public_keys=[]):
		...
		for key in public_keys:
			if key.replace("\n", "") not in output:
				os.system(f"{sudo_option}echo '{key}' >> {HOME}/{username}/.ssh/authorized_keys")
		...
		for key in public_keys:
			if key.replace("\n", "") not in output:
				auth_keys.permission.set(permission=600, sudo=sudo)
				ssh_dir.permission.set(permission=700, sudo=sudo)
				return utils.__error_response__(f'Failed to enable public key [{key.replace("\n", "")}].')
		...
		# success.
		return utils.__success_response__(f'Successfully enabled {len(public_keys)} public key(s).')
	...
	def disable_keys(self, username=None, public_keys=[]):
		...
		for key in output:
			if key.replace("\n", "") not in public_keys:
				if new_keys == "":
					...
					new_keys += key+"\n"
		utils.__save_file__(f"{HOME}/{username}/.ssh/authorized_keys", new_keys)
		...
		for key in public_keys:
			if key.replace("\n", "") in output:
				auth_keys.permission.set(permission=600, sudo=sudo)
				ssh_dir.permission.set(permission=700, sudo=sudo)
				return utils.__error_response__(f'Failed to disable public key [{key.replace("\n", "")}].')
		...
		# success.
		return utils.__success_response__(f'Successfully disabled {len(public_keys)} public key(s).')
...
class Agent(object):
	...
	def add(self, 
		...
		if smart_card or passphrase != None:
			self.initialize()
			l_response = self.list()
			if l_response["error"] != None: return l_response
			count = len(l_response["keys"])
			print("count 1:",count)
			if smart_card:
				if OS in ["osx"]:
			...
			try:
				...
				response["error"] = f"Failed to add key [{path}] to the ssh agent #1."
				return response
			except pexpect.exceptions.EOF:
				output = spawn.read().decode()
				output1 = utils.__execute__(command=["ssh-add", "-l"])
				if ("Failed to communicate" in output1 or "Error connecting to agent" in output1) or ("Error connecting to agent: No such file or directory" in output):
					response["error"] = f"Failed to communicate with the ssh agent. You may need to logout the current system user and log back in."
					return response
				else:
					response["error"] = f"Failed to add key [{path}] to the ssh agent #2."
					return response
			...
			l_response = self.list()
			if l_response["error"] != None: return l_response
			print("count 2:",len(l_response["keys"]))
			if count != len(l_response["keys"]):
				response["success"] = True
				response["message"] = f"Successfully added key [{path}] to the ssh agent."
				return response
			else: 
				response["error"] = f"Failed to add key [{path}] to the ssh agent."
				return response
