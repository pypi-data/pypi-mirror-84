...
class SmartCards(object):
		...
		for card in output:
			if card not in [""]:
				try: 
					...
					print(f"Smart card [{serial_number}] detected.")
	...
	def find_smart_card(self, serial_number=None):
		...
		if response["error"] != None: return response
			...
			return utils.__error_response__(f"There is no smart card detected  with serial number [{serial_number}].")

		# success.
		smart_card = response["smart_cards"][str(serial_number)]
		return utils.__success_response__(f"Successfully initialzed smart card [{serial_number}].", {
...
class SmartCard(object):
	...
	def __init__(self, serial_number=None):	
		...
		if OS in ["linux"]: 
			...
			for path in [
				...
				"/usr/local/lib/libykcs11/opensc-pkcs11.so",
			]:
	...
	def install(self, 
		...
		# initialize.
		response = utils.__default_response__()
		if pin == None: pin = utils.__generate_pincode__(characters=6)
		if puk == None: puk = utils.__generate_pincode__(characters=8)
		info = {
			"pin":pin,
			"puk":puk,
			"public_key":None,
