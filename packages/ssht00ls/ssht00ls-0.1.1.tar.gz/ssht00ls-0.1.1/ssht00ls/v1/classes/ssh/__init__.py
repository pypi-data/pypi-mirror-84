...
class Agent(object):
	def __init__(self):
		...
		#
	def add(self, path=None, smart_card=False, passphrase=None, timeout=0.5):
		...
		if passphrase != None:
			...
			if smart_card:
				output = utils.__execute__(command=["ssh-add", "-K", path], input=passphrase, shell=False, timeout=timeout)
			else:
	...
	def list(self):
		...
		if "Failed to communicate" in output:
			response["error"] = "Failed to communicate with the ssh-agent."
			return response
		elif output == "The agent has no identities":
			response["keys"] = []
		else:
...
response = key.create_config(self, 
	# the servers name.
	server="myserver", 
	# the username.
...
response = agent.add("/path/to/mykey/private_key", passphrase="TestPass!")
