...
class SmartCard(object):
		...
		if OS in ["linux"]: 
			...
			for path in [
				...
				if os.path.exists(self.key_path):
					self.key_path = path
					break
			if self.key_path == None: raise ValueError("Unable to locate opensc-pkcs11.so path.")
					
		elif OS in ["osx"]: 
			self.key_path = "/usr/local/lib/libykcs11.dylib"
	...
	def export_keys(self, 
		...
		# output.
		response = utils.__default_response__()
		command = f"ssh-keygen -D {self.key_path} -e"
