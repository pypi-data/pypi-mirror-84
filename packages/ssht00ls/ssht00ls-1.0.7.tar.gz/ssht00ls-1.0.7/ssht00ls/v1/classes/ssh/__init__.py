	...
	def add(self, 
		...
		if smart_card or passphrase != None:
			...
			try:
				...
				if "Failed to communicate" in output or "agent refused operation" in output or "Error connecting to agent" in output:
					if reattempt:
						try:
			...
			elif "Failed to communicate" in output or "agent refused operation" in output or "Error connecting to agent" in output:
				if reattempt:
					try:
						output = subprocess.check_output([f"ssh-add", "-D"])
						print("delete output:",output)
