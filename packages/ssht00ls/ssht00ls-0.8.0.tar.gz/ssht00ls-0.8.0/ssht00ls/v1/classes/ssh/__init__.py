	...
	def add(self, 
		...
		if smart_card or passphrase != None:
			...
			if "incorrect passphrase" in output.lower():
				...
				if reattempt:
					try:
						output = subprocess.check_output([f"ssh-add", "-D"])
						print("delete output:",output)
					except: a=1
					try:
						output = subprocess.check_output([f"ssh-add", "-k"])
						print("kill output:",output)
					except: a=1
					try:
						output = subprocess.check_output([f"eval", "`ssh-agent`"])
...
class SSHD(object):
	def __init__(self,
		...
		self.banner = Files.File(f"{HOME_BASE}")
		a=1
	def create(self):

		# check users.
		response = self.__check_user_items__(self.users)
		if response["error"] != None: return response
		...
		configuration += '\nLoginGraceTime 15m'
		configuration += '\nMaxAuthTries 3'
		configuration += '\nMaxSessions {}'.format(max_sessions)
		configuration += "\nMaxStartups "+str(max_start_ups)
		configuration += '\nLogLevel {}'.format(log_level)
		configuration += '\nPort {}'.format(server_port_lan)
		...
		configuration += '\nListenAddress {}'.format(listen_address)
		...
		#configuration += '\nChallengeResponseAuthentication {}'.format('no')


		# per users.
		for username, info in self.users.items():
			configuration += '\n# Client: {}'.format(username)
			...
			# root permission.
			l = "no"
			if info["root_permission"] and key_authentication: l = "prohibit-password"
			elif info["root_permission"]: l = "yes"
			configuration += f'\n     PermitRootLogin {l}'
			...
			if info["ip_filter"]:
				...
				# match verified ips.
				configuration += f'\nMatch User {username} {self.__sum_list__(info["allowed_ips"])}'
	...
	def __check_utils_installed__(self, usernames=[]):
		...
		for username in usernames:
			...
			# check version.
				...
				version = utils.__execute__(["sudo", "cat", path])
				if str(version) != str(Files.File(path=f'{SOURCE_PATH}/{VERSION}/classes/utils/.version', load=True).data):
					to_install.append(username)
	...
	def __install_utils__(self, usernames=[]):
		...
		os.system("rm -fr ~/Downloads/ssht00ls/")
		os.system('git clone https://github.com/vandenberghinc/ssht00ls ~/Downloads/ssht00ls/')
		if not os.path.exists("~/Downloads/ssht00ls/"):
			return r3sponse.error_response("Failed to install the ssht00ls utils.")
		os.system(f"mv ~/Downloads/ssht00ls/ssht00ls/v1/classes/utils/ {path}")
		...
		for username in usernames:
			...
			os.system(f"sudo cp -r {path} {HOME_BASE}{username}/.ssh/utils/")
...
# --------------------
# SSHD Config.
#sshd.__check_utils_installed__(["administrator"])
sshd.__install_utils__(["administrator"])
quit()
"""
