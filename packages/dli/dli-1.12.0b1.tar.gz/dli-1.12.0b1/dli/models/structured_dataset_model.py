...
from dli.aws import match_partitions
...
class StructuredDatasetModel(DatasetModel):
    ...
    def __custom_filter(self, partitions: Union[str, List[str]], paths: List[Tuple[str, int]]) -> List[Tuple[str, int]]:

        if partitions:
                return [path for path in paths if match_partitions(path[0], partitions)]
        ...
        if filter_path is not None:
            ...
            if partitions is not None:
                ...
                self._client.logger.warn(
                    f'Partitions and filter_path are set. Filter_path will be '
                    f'ignored as partitions supersedes this.'
                )
    ...
    def download(
        ...
        if filter_path is not None:
            ...
            if partitions is not None:
                filter_path = None
                self._client.logger.warn(
                    f'Partitions and filter_path are set. Filter_path will be ignored as partitions supersedes this.'
                )
        ...
        return self._DatasetModel__download(
            ...
            flatten=flatten,
            filter_path=filter_path,
            custom_filter_func=partial(self.__custom_filter, partitions),
