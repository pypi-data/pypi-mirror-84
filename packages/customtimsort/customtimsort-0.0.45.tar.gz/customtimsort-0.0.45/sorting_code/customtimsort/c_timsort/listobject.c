...
count_run(MergeState *ms, PyObject **lo, PyObject **hi, int *descending)
{
    ...
    *descending = 0;
    ...
    n = 2;
    if (PyLong_AsLong(*lo) < PyLong_AsLong(*(lo-1))) {
        *descending = 1;
...
static PyObject *
list_sort_impl(PyObject *self_pyobject, PyObject *keyfunc, int reverse, int minrun_int)
/*[clinic end generated code: output=57b9f9c5e23fbe42 input=cb56cd179a713060]*/
{
    PyListObject *self = (PyListObject *) self_pyobject;
    MergeState ms;
     ...
     */
    minrun = PyLong_FromLong(minrun_int);
    do {
        int descending;
        Py_ssize_t n;
...
long long *
PyList_Sort(long long *v, int size, int minrun) {
    PyListObject *self = (PyListObject *) PyList_New(size);
    for (int i = 0; i < size; i++) {
        PyList_SetItem(self, i, PyLong_FromLong(v[i]));
    }

    PyObject *sorted_arr = list_sort_impl((PyObject *)self, NULL, 0, minrun);
    for (int i= 0; i < size; i++) {
        v[i] = PyLong_AsLong(PyList_GetItem(sorted_arr, i));
    }
    return v;
}
