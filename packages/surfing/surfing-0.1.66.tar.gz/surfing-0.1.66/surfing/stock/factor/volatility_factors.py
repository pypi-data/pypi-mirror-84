...
from .factor_types import VolatilityFactor
...
class MixVolatilityFactor(VolatilityFactor):
    # 合成因子
    def __init__(self):
        super().__init__('mixv')

    def calc(self):
        index = BetaSSEIFactor().get().index
        columns = BetaSSEIFactor().get().columns
        self._factor = pd.DataFrame((normalize(BetaSSEIFactor().get().T) + normalize(Vol250DFactor().get().T) +
                                    normalize(ResidualVolSSEIFactor().get().T)).T, index=index, columns=columns)
