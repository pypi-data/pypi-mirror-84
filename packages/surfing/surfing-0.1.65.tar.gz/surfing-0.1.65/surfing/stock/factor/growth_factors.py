...
class MixGrowthFactor(GrowthFactor):
    ...
    def calc(self):
        index = IncomeGrowthFactor().get().index
        columns = IncomeGrowthFactor().get().columns
        self._factor = pd.DataFrame((normalize(IncomeGrowthFactor().get().T) + normalize(NetProfitOfNonRecurringGoLFactor().get().T) +
                                    normalize(OperationalCashFlowFactor().get().T)).T, index=index, columns=columns)
