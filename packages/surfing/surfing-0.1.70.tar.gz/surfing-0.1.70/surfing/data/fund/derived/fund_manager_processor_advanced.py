...
class ManagerProcessor:
    ...
    def init(self, start_date=str, end_date=str, print_time=False, is_mng_index=False, is_mng_indicator=False):
        ...
        self.fund_manager_size_df[self.fund_manager_size_df < 1e8] = self.PUNISH_RATIO
        self.fund_manager_size_df[self.fund_manager_size_df >= 1e8] = 1
    ...
    def index_calculation(self, m_id: str, fund_type: str):
        ...
        # 基金经理管理的基金 规模状态 大于1e 为 1， 否则为 0.8
    ...
    def calc_score(self):
        ...
        for fund_type in self.FUND_CLASSIFIER:
            ...
            if fund_type == 'stock':
                ...
                df.loc[:,'experience'] = score_rescale( 0.8 *  score_rescale(df['fund_type_trading_days']) + 0.2 * score_rescale(df['fund_size']))
                ...
                df.loc[:,'risk_ability'] = score_rescale(0.3 * score_rescale(-df['mdd~history_m']) + 0.4 * score_rescale(-df['annual_vol~history_m']) + 0.3 * score_rescale(-df['downside_std~history_m']))
                df.loc[:,'experience'] = score_rescale( 0.4 *  score_rescale(df['fund_type_trading_days']) + 0.6 * score_rescale(df['fund_size']))
        ...
        _df = self.fund_manager_info.reset_index().rename(columns={'manager_id':'mng_id'})[['mng_id','name','company_id']].drop_duplicates().set_index('mng_id')
