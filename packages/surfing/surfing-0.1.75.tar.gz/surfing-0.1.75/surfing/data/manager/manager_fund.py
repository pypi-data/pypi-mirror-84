...
class FundDataManager(DataManager):
    ...
    def fund_analysis_by_stock_factors(self, fund_id: str, factor_names: Tuple[str], universe: str = 'default') -> Optional[pd.DataFrame]:
        '''对基金计算因子暴露'''
        try:
            try:
    ...
    def fund_pos_analysis_by_stock_factors(self, fund_id: str, factor_names: Tuple[str], report_date: datetime.date, universe: str = 'default') -> Optional[pd.DataFrame]:
        '''对基金持仓计算因子暴露'''
        return StockFactorApi.fac_pos(fund_id, factor_names, report_date, universe)
