...
class GLXCd(cmd2.CommandSet):
        ...
        if self.can_continue and os.environ.get("CDPATH"):
            for pathname in os.environ.get("CDPATH").split(":"):
                ...
                # the operand names a directory.
                    ...
                    if os.path.isdir(os.path.join("../../cmds", self.directory)):
                        self.curpath = os.path.join("../../cmds", self.directory)
                        self._step_7()
                        break

                # In either case, if the resulting string names an existing directory,
                else:
                    if pathname is not None and os.path.isdir(pathname):
    ...
    def _step_7(self):
        ...
        if self.can_continue and self.physical:
            ...
            elif not os.access(self.curpath, os.R_OK):
    ...
    def do_cd(self, args):
        ...
        self.cd(directory=args.directory, logical=args.logical, physical=args.physical)
