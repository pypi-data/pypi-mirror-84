...
def predict_stock_price_by_option(ticker,lastndays=7,power=4):
    ...
    print("   2) +/- predicts higher/lower price than recent one, ++/-- means more likely.")
...
def compare_otm(row):
    ...
    if abs(row['OTM Calls in Total%'] - row['OTM Puts in Total%']) < 0.1:
        result='+/-'
...
def recent_stock_split(ticker):
    ...
    if len(div2)==0:
        print("#Warning(stock_split): no split info in period",fromdate,today)
        return None          
...
def get_last_close(ticker):
    ...
    try:
        ...
        print("#Error(get_last_close): failed in retrieving prices!")        
        return None            
    if price is None:
        print("#Error(get_last_close): retrieved none info!")
        return None  
    if len(price)==0:
        print("#Error(get_last_close): retrieved empty info!")
        return None         
