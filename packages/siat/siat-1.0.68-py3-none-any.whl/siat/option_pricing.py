...
def int2date(intdate):
    ...
    if isinstance(intdate,str):
        standardTime = intdate[:10]
        return standardTime      
    ...
    print("#Error(int2date): integer date must be 10 or 13 digits,",intdate)
    return None
...
def predict_stock_price_by_option(ticker,lastndays=7,power=4):
    """
    ...
    for d in datelist:
        print("Processing options with maturity date",d)
    ...
    df2x['+/-']=df2x.apply(compotm,axis=1)  #axis=1表示按行操作
    collist=['Date','+/-','WA Strike','Total Options','Calls in Total%','OTM in Calls%', \
            'Puts in Total%','OTM in Puts%']
    df3=df2x[collist]
    df3.rename(columns={'Calls in Total%':'Calls%','Puts in Total%':'Puts%'}, inplace = True)
    
    print("\n ======= Predicting Stock Prices via Stock Option Configuration: "+ticker+" =======")
    #设置打印对齐
    ...
    print(" Note:")
    print(" 1) +/- predicts stock price will be higher/lower than today;")
    print(" 2) Using last "+str(lastndays)+" days' options for sampling;")
    print(" 3) Sampling period should not include stock splits;")
    print(" 4) Removed those samples with only one option, if any;")
    print(" 5) Calculating WA Strike by volume weighted average of strikes.")
    print(" "+footnote)
