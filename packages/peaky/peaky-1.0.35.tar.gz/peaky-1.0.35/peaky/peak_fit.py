...
def fit_voigt(x, y, peak_ratio = 0.1, background = 1, a = None, p = None, g = None, l = None, log = True):   
    ...
    functions = [lambda nu: signal_model(nu, *_old_var(fit, var)), lambda nu: voigt_model(nu, *_old_var(fit, var)[:4]), lambda nu: background_model(nu, *_old_var(fit, var)[4:])]
    ...
    iniz.insert(4, _full_width(iniz[2], iniz[3]))
    fit.insert(4, _full_width(fit[2], fit[3]))
    err.insert(4, _full_width_error(fit[2], fit[3], err[2], err[3]))
    ...
    return fit, err, snr, functions, data
