...
.. code-block:: python
   ...
   # you can pass the maximum blocking time (s) allowed (return False 
   # if every item hasn't be refreshed)
   >>> group.read(3.0)
   >>> True
   >>> group.table()
       +----+--------+-------+-------------------------+-------+------+
...
A group object is iterable and accessable as an array, alloing you to process items one by one. There are some useful other methods
