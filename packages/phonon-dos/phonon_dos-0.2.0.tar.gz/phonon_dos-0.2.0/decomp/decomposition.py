...
Nqs_path, ks_path, ks_scaled_path, kk, Hk = util.get_commensurate_kpath(file_initial_conf, N1,N2,N3, N1,N2,N3, kinput_scaled)
...
Ds = np.zeros((len(irr_kpoints),tot_atoms_uc*3, tot_atoms_uc*3), dtype=complex)
for i in range(len(irr_kpoints)):
    k = irr_kpoints[i]
    k_scal = irr_kpoints_scal[i]
    thisk_permutations = permutations_types[i]
    weight = weights[i]
    print('\t irr kpoint scaled: ', i, k_scal)
    print('\t weight: ', weight)
    
# =============================================================================
# harmonic things
    D = util.get_D(k, K,N1,N2,N3,R0,masses_uc, tot_atoms_uc, SCell)
    eigvals, eigvec = np.linalg.eigh(D)
    idx = np.argsort(eigvals)
    eigvals = eigvals[idx]
    omegas = np.sqrt(eigvals)/2/np.pi
    eigvec, eigvecH = eigvec[:,idx], np.conjugate(eigvec[:,idx].T)
# =============================================================================
...
#  renormalised D
    ...
    D_renorm = np.dot(np.dot(eigvec,Lamdas_renorm),eigvecH)
...
# K_renormold = util.get_K_renorm_old(tot_atoms_uc,N1,N2,N3, SCell, R0, kpoints, Ds)
