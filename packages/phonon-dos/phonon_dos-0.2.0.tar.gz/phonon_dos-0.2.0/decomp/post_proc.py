...
if(plot_type==4):
    ...
    # ks_path, x_labels, distances_comm = read.read_path(kinput, Nqs_path, labels, Hk)
    ks_scal_path_many, ks_path_many, distances = util.get_kpath_many(kinput, Hk, npoints=interp)
...
if(plot_type==5):
    ...
    ks_scal_path_many, ks_path_many, distances_many = util.get_kpath_many(kinput, Hk, npoints=interp)
    num_modes = len(modes)
    K_renorm = np.loadtxt('K_renorm')
...
# =============================================================================      
    ...
    if (interp>0):
        ZQS_proj = util.interpol_spectrum(ks_path, freq, freqs_from_spectrum, distances_comm, distances_many, frequencies, EIGVECS, modes, ZQS_proj_reshaped)
    else:
        ZQS_proj = ZQS_proj_reshaped[:,range(len(modes)),:]
        
    plot.plot_k2(distances_comm, distances_many, freqs_disp_reshaped, freqs_reshaped, freq,ZQS,ZQS_proj, x_labels,max_Z,modes)
