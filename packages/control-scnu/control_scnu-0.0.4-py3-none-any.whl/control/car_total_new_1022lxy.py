...
# name=0  #存储异常识别的情况
car_1 = gpio.car(1, 2)
car_1.setspeed('go',23,23)
car_1.setspeed('turn_left',3,23)
car_1.setspeed('turn_right',23,3)
...
while 1:
    ...
    if camera == 1 and park_enter == 0:  # camera==1,说明是遇到等待线了, prak_enter=0,说明还没进入停车场，所以只能是十字路口，红绿灯、语音控制区
        ...
        cv2.imshow('img', img)
        cv2.waitKey(40)
        img_path_pre = '/home/pi/camera_pos/0.jpg'
        ...
        if i == 20:
            ...
            if red_is_gone==0:
                red.detect(img)  # 检测红色
                red_detect = red.data
            img_path_color = '/home/pi/camera_pos/'+red_detect+'/'+str(j_color)+'.jpg'
            ...
            #保存识别到的数字方向图片到对应的文件夹
            img_path_mdir = '/home/pi/camera_pos/'+mdir.data+'/'+str(j)+'.jpg'
        ...
        print('i:',i)
        if i == 23:
            img_path_pre = '/home/pi/camera_pos/3.jpg'
            cv2.imwrite(img_path_pre, img_new)
            frame = Image.open(img_path_pre)
            ...
            cv2.destroyAllWindows()
            #保存停车场拍摄的照片
            img_path_num = '/home/pi/camera_pos/'+num.data+'/'+str(j)+'.jpg'
