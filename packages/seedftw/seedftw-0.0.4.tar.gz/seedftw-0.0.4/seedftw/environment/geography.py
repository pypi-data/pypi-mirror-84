...
def distance_between_coordinates(coordinates_1,coordinates_2):
    ...
    K2 = 111.41513*cosd(latitude_M) - 0.09455*cosd(3*latitude_M) + 0.00012*cosd(5*latitude_M)
    
    distance = np.sqrt((K1*(latitude_1 - latitude_2))**2 + (K2*(longitude_1 - longitude_2))**2)
    
    if distance>=475:
        print("WARNING: The distance estimation is not accurate at this distance (" + str() + " km > 475 km)")
        distance = np.NAN
