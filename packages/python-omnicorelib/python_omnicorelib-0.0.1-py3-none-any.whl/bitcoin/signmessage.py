...
from omnicore.core.key import CPubKey
from omnicore.core.serialize import ImmutableSerializable
from omnicore.wallet import P2PKHBitcoinAddress
import omnicore
import base64
import sys
...
class BitcoinMessage(ImmutableSerializable):
    ...
    def stream_deserialize(cls, f):
        magic = omnicore.core.serialize.BytesSerializer.stream_deserialize(f)
        message = omnicore.core.serialize.BytesSerializer.stream_deserialize(f)
        return cls(message, magic)

    def stream_serialize(self, f):
        omnicore.core.serialize.BytesSerializer.stream_serialize(self.magic, f)
        omnicore.core.serialize.BytesSerializer.stream_serialize(self.message, f)
