"""
`FakeBitcoinProxy` allows for unit testing of code that normally uses omnicore
RPC without requiring a running omnicore node.

`FakeBitcoinProxy` has an interface similar to `omnicore.rpc.Proxy`, but does not
connect to a local omnicore RPC node. Hence, `FakeBitcoinProxy` is similar to a
mock for the RPC tests.

`FakeBitcoinProxy` does _not_ implement a full omnicore RPC node. Instead, it
...
from omnicore.core import (
    ...
    b2lx,
    # number of satoshis per omnicore
    COIN,
...
from omnicore.wallet import (
    # omnicore address initialized from base58-encoded string
    CBitcoinAddress,
...
class FakeBitcoinProxy(object):
    """
    This is an alternative to using `omnicore.rpc.Proxy` in tests. This class
