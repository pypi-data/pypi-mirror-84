...
is in omnicore.core.scripteval
"""
...
class CScript(bytes):
    ...
    def __coerce_instance(cls, other):
        ...
        if isinstance(other, CScriptOp):
            ...
            if 0 <= other <= 16:
                ...
                other = CScriptOp.encode_op_pushdata(omnicore.core._bignum.bn2vch(other))
    ...
    def __repr__(self):
        ...
        def _repr(o):
            if isinstance(o, bytes):
                return "x('%s')" % omnicore.core.b2x(o)
            else:
                return repr(o)
    ...
    def is_p2sh(self):
        ...
        return (
            len(self) == 23
            and _bord(self[0]) == OP_HASH160
            and _bord(self[1]) == 0x14
            and _bord(self[22]) == OP_EQUAL
        )
    ...
    def to_p2sh_scriptPubKey(self, checksize=True):
        ...
        return CScript([OP_HASH160, omnicore.core.Hash160(self), OP_EQUAL])
...
class CScriptWitness(ImmutableSerializable):
    ...
    def __repr__(self):
        return (
            "CScriptWitness("
            + ",".join("x('%s')" % omnicore.core.b2x(s) for s in self.stack)
            + ")"
        )
...
def RawSignatureHash(script, txTo, inIdx, hashtype):
    ...
    txtmp = omnicore.core.CMutableTransaction.from_tx(txTo)
    ...
    if (hashtype & 0x1F) == SIGHASH_NONE:
        ...
        for i in range(outIdx):
            txtmp.vout.append(omnicore.core.CTxOut())
    ...
    txtmp.wit = omnicore.core.CTxWitness()
    s = txtmp.serialize()
    s += struct.pack(b"<i", hashtype)

    hash = omnicore.core.Hash(s)
...
def SignatureHash(
    ...
    if sigversion == SIGVERSION_WITNESS_V0:
        ...
        if not (hashtype & SIGHASH_ANYONECANPAY):
            ...
            hashPrevouts = omnicore.core.Hash(serialize_prevouts)
        ...
        if (
            ...
            hashSequence = omnicore.core.Hash(serialize_sequence)

        if (hashtype & 0x1F) != SIGHASH_SINGLE and (hashtype & 0x1F) != SIGHASH_NONE:
            serialize_outputs = bytes()
            for o in txTo.vout:
                serialize_outputs += o.serialize()
            hashOutputs = omnicore.core.Hash(serialize_outputs)
        ...
        return omnicore.core.Hash(f.getvalue())
