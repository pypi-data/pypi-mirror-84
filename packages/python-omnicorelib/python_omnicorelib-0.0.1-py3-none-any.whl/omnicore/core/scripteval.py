...
import omnicore.core.serialize
...
class EvalScriptError(omnicore.core.ValidationError):
...
def _CastToBigNum(s, err_raiser):
    v = omnicore.core._bignum.vch2bn(s)
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(omnicore.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(omnicore.core._bignum.bn2vch(v))
                ...
                check_args(1)
                stack.append(omnicore.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(omnicore.core.serialize.Hash(stack.pop()))
...
class VerifyScriptError(omnicore.core.ValidationError):
    pass
...
class VerifySignatureError(omnicore.core.ValidationError):
    pass
