...
from omnicore.core.serialize import *
from omnicore.net import *
import omnicore
...
class MsgSerializable(Serializable):
    ...
    def to_bytes(self):
        ...
        self.msg_ser(f)
        body = f.getvalue()
        res = omnicore.params.MESSAGE_START
        res += self.command
    ...
    def stream_deserialize(cls, f, protover=PROTO_VERSION):
        ...
        # check magic
        if recvbuf[:4] != omnicore.params.MESSAGE_START:
            raise ValueError(
                "Invalid message start '%s', expected '%s'"
                % (b2x(recvbuf[:4]), b2x(omnicore.params.MESSAGE_START))
            )
...
class msg_version(MsgSerializable):
    ...
    def __init__(self, protover=PROTO_VERSION):
        ...
        self.strSubVer = (
            b"/python-bitcoinlib:" + omnicore.__version__.encode("ascii") + b"/"
        )
