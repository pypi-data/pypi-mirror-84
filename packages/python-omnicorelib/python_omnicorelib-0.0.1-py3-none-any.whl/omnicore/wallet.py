...
import omnicore
import omnicore.base58
import omnicore.bech32
import omnicore.core
import omnicore.core.key
import omnicore.core.script as script
...
class CBitcoinAddress(object):
    def __new__(cls, s):
        ...
        except omnicore.bech32.Bech32Error:
            pass

        try:
            return CBase58BitcoinAddress(s)
        except omnicore.base58.Base58Error:
            pass

        raise CBitcoinAddressError("Unrecognized encoding for omnicore address")
...
class CBech32BitcoinAddress(omnicore.bech32.CBech32Data, CBitcoinAddress):
    ...
    def from_bytes(cls, witver, witprog):
        ...
        assert witver == 0
        self = super(CBech32BitcoinAddress, cls).from_bytes(witver, _tobytes(witprog))
...
class CBase58BitcoinAddress(omnicore.base58.CBase58Data, CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion):
        ...
        if nVersion == omnicore.params.BASE58_PREFIXES["SCRIPT_ADDR"]:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == omnicore.params.BASE58_PREFIXES["PUBKEY_ADDR"]:
...
class P2SHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = omnicore.params.BASE58_PREFIXES["SCRIPT_ADDR"]

        elif nVersion != omnicore.params.BASE58_PREFIXES["SCRIPT_ADDR"]:
            raise ValueError(
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            return cls.from_bytes(
                scriptPubKey[2:22], omnicore.params.BASE58_PREFIXES["SCRIPT_ADDR"]
            )
    ...
    def to_scriptPubKey(self):
        ...
        assert self.nVersion == omnicore.params.BASE58_PREFIXES["SCRIPT_ADDR"]
...
class P2PKHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = omnicore.params.BASE58_PREFIXES["PUBKEY_ADDR"]
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        """Create a P2PKH omnicore address from a pubkey
        ...
        if not accept_invalid:
            if not isinstance(pubkey, omnicore.core.key.CPubKey):
                pubkey = omnicore.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError("invalid pubkey")

        pubkey_hash = omnicore.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(
        ...
        if accept_non_canonical_pushdata:
            ...
            except omnicore.core.script.CScriptInvalidError:
                raise CBitcoinAddressError(
        ...
        if scriptPubKey.is_witness_v0_keyhash():
            return cls.from_bytes(
                scriptPubKey[2:22], omnicore.params.BASE58_PREFIXES["PUBKEY_ADDR"]
            )
        elif scriptPubKey.is_witness_v0_nested_keyhash():
            return cls.from_bytes(
                scriptPubKey[3:23], omnicore.params.BASE58_PREFIXES["PUBKEY_ADDR"]
            )
        elif (
            len(scriptPubKey) == 25
    ...
    def to_scriptPubKey(self, nested=False):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == omnicore.params.BASE58_PREFIXES["PUBKEY_ADDR"]
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = omnicore.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = omnicore.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(omnicore.base58.Base58Error):
    pass


class CBitcoinSecret(omnicore.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(
            ...
            omnicore.params.BASE58_PREFIXES["SECRET_KEY"],
        )
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != omnicore.params.BASE58_PREFIXES["SECRET_KEY"]:
            raise CBitcoinSecretError(
                "Not a base58-encoded secret key: got nVersion=%d; expected nVersion=%d"
                % (self.nVersion, omnicore.params.BASE58_PREFIXES["SECRET_KEY"])
            )
