...
class BaseProxy(object):
    ...
    def __init__(
        ...
        if service_url is None:
            # Figure out the path to the omnicore.conf file
            if btc_conf_file is None:
                ...
                btc_conf_file = os.path.join(btc_conf_file, "omnicore.conf")

            # Bitcoin Core accepts empty rpcuser, not specified in btc_conf_file
            conf = {"rpcuser": ""}

            # Extract contents of omnicore.conf to build service_url
            try:
            ...
            # Treat a missing omnicore.conf as though it were empty
            except FileNotFoundError:
                pass

            if service_port is None:
                service_port = omnicore.params.RPC_PORT
            ...
            if omnicore.params.NAME != "mainnet":
                cookie_dir = os.path.join(cookie_dir, omnicore.params.NAME)
...
class RawProxy(BaseProxy):
    """Low-level proxy to a omnicore JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function omnicore.rpc.name> rather than <function
        # omnicore.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    """Proxy to a omnicore RPC service

    Unlike ``RawProxy``, data is passed as ``omnicore.core`` objects or packed
    ...
    def __init__(
        ...
        **kwargs
        ...
        specified, ``~/.omnicore/omnicore.conf`` or equivalent is used by
    ...
    def generate(self, numblocks):
        """
        DEPRECATED (will be removed in omnicore-core v0.19)
    ...
    def getblock(self, block_hash):
        ...
        try:
            ...
            # omnicore core's rpc takes 0/1/2 instead of true/false as the 2nd argument which specifies verbosity, since v0.15.0.
