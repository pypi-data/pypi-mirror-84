...
from tktl.cli.common import ClickGroup, logger, to_uuid
...
def delete_deployment_by_id(deployment_id: UUID4):
    ...
    if inp != "y":
        logger.error("Safe choice!")
    command = DeleteDeployment()
    try:
        response = command.execute(deployment_id=deployment_id)
    except APIClientException:
        logger.error("Only super users have access to this method at this time")
        return
    logger.warning(f"Deployment with id: {response.id}")
    return response
