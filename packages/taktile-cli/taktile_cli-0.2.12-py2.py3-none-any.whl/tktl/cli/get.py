...
from tktl.cli.common import ClickGroup, get_cmd_shared_options, logger, to_uuid
...
def produce_output(
    ...
    if output_kind == "stdout":
        ...
        logger.log(GenericResourceList.parse_obj(resources).json())
    elif output_kind == "yaml":
        logger.log(yaml.dump_all([json.loads(r.json()) for r in resources]))
