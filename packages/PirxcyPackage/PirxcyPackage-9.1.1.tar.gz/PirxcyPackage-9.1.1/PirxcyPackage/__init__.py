...
print(crayons.cyan(f'[PirxcyBot] [{time()}] Made For {nickname}Coded By Pirxcy.'
                   'Sub To Pirxcy on. YT.'))
...
async def event_party_member_join(member: fortnitepy.PartyMember) -> None:
    delay.sleep(0)
    await client.party.send(f'To Get Your Own Bot Join The Discord On - discord.gg/wxEBMTZ\n\Coded/Made By Pirxcy ')
...
@client.command()
async def emleaks(ctx: fortnitepy.ext.commands.Context) -> None:
    async with aiohttp.ClientSession() as session:
        request = await session.request(
            method='GET',
            url='https://benbotfn.tk/api/v1/files/added',
    ...
    await ctx.send(f'Displayed All Leaked Emoji')
